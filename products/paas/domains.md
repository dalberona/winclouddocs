...
description: Domain managing through WinstarCloud Cloud
registerDomain:
    0:
...
white-labeling:
    ...
    1:
        ...
        title: 'Now you can use your domain name to access WinstarCloud Cloud Web UI and services. Try to login by entering the chosen domain name in the browser address line.'
...
domainDetails:
    ...
    1:
        ...
        title: 'WinstarCloud Cloud will automatically renew the certificate earlier than 30 days before it expires, no action is required from your part unless you change or delete the domain CNAME record.'
...
The [WinstarCloud Cloud](https://winstarcloud.cloud/signup) allows registering your custom domain to have the required host name for user’s access, links, etc. 
When you register valid domain, the WinstarCloud Cloud automatically requests the SSL certificate from [Let's Encrypt](https://letsencrypt.org/) for the latter and manages further certificate renewals. 
...
Like Web UI all other WinstarCloud Cloud services such as MQTT/HTTP/CoAP transports or HTTP integrations will be accessible via your custom domain name.
...
In order to use your own host name instead of **winstarcloud.cloud** you must register it first. Follow the next steps to do that:

* On your DNS provider’s website, you must add a canonical record for your domain to map it with **winstarcloud.cloud**. See [How to Create a CNAME Record For Your Domain](#how-to-create-a-cname-record) for details. Once done, you can start the registration procedure;
...
Now you can use your domain name to access WinstarCloud Cloud Web UI and services. Try to login by entering the chosen domain name in the browser address line.
...
The certificate is valid for 90 days. Please note that WinstarCloud Cloud will automatically renew the certificate earlier than 30 days before it expires, no action is required from your part unless you change or delete the domain CNAME record.
...
For example, $YOUR_DOMAIN_NAME is `mydomain.winstarcloud.online`:
```bash
dig mydomain.winstarcloud.online any
```
...
; <<>> DiG 9.16.1-Ubuntu <<>> mydomain.winstarcloud.online any
;; global options: +cmd
;; Got answer:
...
;; QUESTION SECTION:
;mydomain.winstarcloud.online.	IN	ANY

;; ANSWER SECTION:
mydomain.winstarcloud.online. 3600 IN	HINFO	"RFC8482" ""
...
This output shows that there are NO CNAME added to the mydomain.winstarcloud.online domain ("ANSWER SECTION" block).

Correct output should look like that:
```bash
...
;; ANSWER SECTION:
mydomain.winstarcloud.online. 3600 IN	CNAME	winstarcloud.cloud
...
```

If all the things are correct, but some issue still persists - please [Contact us](https://winstarcloud.io/docs/contact-us/) for further support.
