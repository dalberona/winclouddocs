...
assignees:
- ashvayka
title: What is WinstarCloud License Server?
description: Features and advantages of WinstarCloud License Server

---


The **WinstarCloud License Server** is a proprietary billing solution that allows **WinstarCloud Professional Edition (TB PE)** customers to easily purchase license keys online.
...
The License Server was introduced in mid-2019, based on our experience of billing first 500 TB PE clients, to bring improvements in the following areas:
 ...
 - **Cluster setup.** With a modern microservices deployment approach, we added the ability to set up a single license key per all nodes in the WinstarCloud cluster. 
 This minimizes efforts for cluster management and removes manual work required to add/remove nodes in the cluster. 
 Now you can launch several cluster nodes in a [floating mode](https://en.wikipedia.org/wiki/Floating_licensing) and actual WinstarCloud processes are not related to physical hardware.
 
 - **Distributors and Channel Partners.** For WinstarCloud partners License server simplifies management of their clients. 
...
License Server allows purchasing monthly or yearly subscriptions for WinstarCloud. See [pricing](/pricing/) for more details about available subscriptions.
Once you purchase a subscription, you can flexibly upgrade or downgrade your subscription plan. By default, the subscription covers single WinstarCloud PE instance (server process). 
...
A single perpetual fallback license covers a single WinstarCloud PE instance (server process). 
For example, if you like to run WinstarCloud PE in an HA mode, you will need at least two licenses.
...
WinstarCloud has no access to your credit card data. You can cancel your subscription at any time. 
WinstarCloud also provides the ability to download digital copies copy of the invoices.   
...
The License Server Client (e.g. your WinstarCloud PE instance) requires an internet connection to the host: license.winstarcloud.io to issue license check requests. 
In case internet connection to the host is not available for more than 24 hours, License Server Client may shutdown WinstarCloud instance.  
...
During the first launch of WinstarCloud PE, built-in License Server Client generates an "Activate Instance Request" to the License Server. 
...
If those requests are not successful for a configurable period of time (typically 24 hours), the license client will shut down the WinstarCloud PE instance.
