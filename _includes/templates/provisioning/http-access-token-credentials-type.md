...
def collect_required_data():
    config = {}
    print("\n\n", "="*80, sep="")
    print(" "*10, "\033[1m\033[94mWinstarCloud device provisioning with access token authorization example script. HTTP API\033[0m", sep="")
    print("="*80, "\n\n", sep="")
    host = input("Please write your WinstarCloud \033[93murl\033[0m or leave it blank to use default (https://winstarcloud.cloud): ")
    config["host"] = host if host else "https://winstarcloud.cloud"
    port = input("Please write your WinstarCloud \033[93mHTTP port\033[0m or leave it blank to use default (443): ")
...
if __name__ == '__main__':
    ...
    response = post("%s:%i/api/v1/provision" % (WINSTARCLOUD_HOST, WINSTARCLOUD_PORT), json=PROVISION_REQUEST)
    decoded_response = response.json()
    print("Received response: ")
    print(decoded_response)
    received_token = decoded_response.get("credentialsValue")
    if received_token is not None:
        response = post('%s:%i/api/v1/%s/telemetry' % (WINSTARCLOUD_HOST, WINSTARCLOUD_PORT, received_token,), dumps(to_publish))
        print("[WINSTARCLOUD CLIENT] Response code from Winstarcloud.")
