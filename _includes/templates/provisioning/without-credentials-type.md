...
WINSTARCLOUD_HOST = "mqtt.winstarcloud.cloud"  # WinstarCloud instance host
WINSTARCLOUD_PORT = 1883  # WinstarCloud instance MQTT port
...
def on_tb_connected(client, userdata, flags, rc):  # Callback for connect with received credentials
    if rc == 0:
        print("[WinstarCloud client] Connected to WinstarCloud with credentials: %s" % client._username)
    else:
...
To communicate with WinstarCloud we will use asyncio and aiocoap modules, so we should install it: <br><br>
...
WINSTARCLOUD_HOST = "127.0.0.1"
WINSTARCLOUD_PORT = "5683"
