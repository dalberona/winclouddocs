...
| username                  | **DEVICE_USERNAME_HERE**                     | Username for device in WinstarCloud.                                            |
| password                  | **DEVICE_PASSWORD_HERE**                     | Password for device in WinstarCloud.                                            |
| clientId                  | **DEVICE_CLIENT_ID_HERE**                    | Client id for device in WinstarCloud.                                           |
|---
...
To communicate with WinstarCloud we will use Paho MQTT module, so we should install it:
...
def collect_required_data():
    config = {}
    print("\n\n", "="*80, sep="")
    print(" "*10, "\033[1m\033[94mWinstarCloud device provisioning with basic authorization example script.\033[0m", sep="")
    print("="*80, "\n\n", sep="")
    ...
    port = input("Please write your WinstarCloud \033[93mport\033[0m or leave it blank to use default (1883): ")
...
class ProvisionClient(Client):
    ...
    def __on_connect(self, client, userdata, flags, rc):  # Callback for connect
        if rc == 0:
            print("[Provisioning client] Connected to WinstarCloud ")
            client.subscribe(self.PROVISION_RESPONSE_TOPIC)  # Subscribe to provisioning response topic
            provision_request = dumps(self.__provision_request)
            print("[Provisioning client] Sending provisioning request %s" % provision_request)
            client.publish(self.PROVISION_REQUEST_TOPIC, provision_request)  # Publishing provisioning request topic
        else:
            print("[Provisioning client] Cannot connect to WinstarCloud!, result: %s" % RESULT_CODES[rc])

    def __on_message(self, client, userdata, msg):
        decoded_payload = msg.payload.decode("UTF-8")
        print("[Provisioning client] Received data from WinstarCloud: %s" % decoded_payload)
    ...
    def provision(self):
        print("[Provisioning client] Connecting to WinstarCloud (provisioning client)")
    ...
    def get_new_client(self):
        ...
        if client_credentials:
            ...
            new_client.username_pw_set(client_credentials["userName"], client_credentials["password"])  # Setting username and password for WinstarCloud client
...
def on_tb_connected(client, userdata, flags, rc):  # Callback for connect with received credentials
    if rc == 0:
        print("[WinstarCloud client] Connected to WinstarCloud with credentials: username: %s, password: %s, client id: %s" % (client._username.decode(), client._password.decode(), client._client_id.decode()))
    else:
        print("[WinstarCloud client] Cannot connect to WinstarCloud!, result: %s" % RESULT_CODES[rc])
...
if __name__ == '__main__':
    ...
    WINSTARCLOUD_PORT = config["port"]  # WinstarCloud instance MQTT port
    ...
    provision_client = ProvisionClient(WINSTARCLOUD_HOST, WINSTARCLOUD_PORT, PROVISION_REQUEST)
    ...
    if tb_client:
        ...
        tb_client.connect(WINSTARCLOUD_HOST, WINSTARCLOUD_PORT, 60)
