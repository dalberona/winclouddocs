...
```bash
curl -v -X POST -d "{\"temperature\": 25}" https://winstarcloud.cloud/api/v1/$ACCESS_TOKEN/telemetry --header "Content-Type:application/json"
```
{: .copy-code}
...
```bash
curl -v -X POST -d "{\"temperature\": 25}" https://winstarcloud.cloud/api/v1/ABC123/telemetry --header "Content-Type:application/json"
```
{: .copy-code}
...
```bash
curl -v -X POST -d "{\"temperature\": 25}" http://$WINSTARCLOUD_HOST_NAME_AND_PORT/api/v1/$ACCESS_TOKEN/telemetry --header "Content-Type:application/json"
```
{: .copy-code}
