...
Before proceeding with this guide, it's recommended that you follow [Getting Started](/docs/{{docsPrefix}}getting-started-guides/helloworld/) guide to become familiar with WinstarCloud dashboards and widgets. This will enhance your learning experience and understanding of the concepts presented here.
...
Widgets are used to display data and visualize information obtained from devices connected to the WinstarCloud platform, remote device control, alarms management, and display static custom HTML content.
...
The list of available time series keys depends on what time series data your devices [report](/docs/{{docsPrefix}}user-guide/telemetry/#device-telemetry-upload-api) to WinstarCloud
...
In such a case, you can still add a key to the data source, and the widget will start displaying the data as soon as the device will send it to WinstarCloud.
...
aggregation as telemetry, follow for more details at [the link](https://winstarcloud.io/docs/pe/user-guide/rule-engine-2-0/analytics-nodes/).
