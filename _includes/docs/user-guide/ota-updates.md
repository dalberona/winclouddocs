...
Since WinstarCloud 3.3, WinstarCloud allows you to upload and distribute over-the-air(OTA) updates to devices. 
...
WinstarCloud will notify devices about the available update and provide a protocol-specific API to download the firmware. 
...
## Provision OTA package to WinstarCloud repository
...
There is a very minor difference between FOTA and SOTA implementation inside WinstarCloud core. Many use-cases and applications will need to use FOTA only. 
...
The device profile details will let you choose only compatible OTA update packages (see [provisioning](#provision-ota-package-to-winstarcloud-repository) for more info).
...
WinstarCloud tracks the progress of the update and persists it to the device attributes.
...
WinstarCloud queues the update notifications to avoid peak loads. The queue is processed with the constant pace. 
...
Under the hood, WinstarCloud converts notification to the update of the following [shared attributes](/docs/{{docsPrefix}}user-guide/attributes/#shared-attributes):
...
Once the firmware/software is updated, WinstarCloud expect the device to send the following telemetry:
...
If the firmware/software update failed, WinstarCloud expect the device to send the following telemetry:
...
WinstarCloud provides the summary of the firmware/software update to monitor and track the firmware/software update status of your device, such as which devices are updating right now, any boot issues, and which ones have already been updated.

### Firmware update monitoring dashboard

The dashboard is created automatically for each new tenant that you add to WinstarCloud. 
You can also download the dashboard JSON [here](https://github.com/winstarcloud/winstarcloud/blob/master/application/src/main/data/json/demo/dashboards/firmware.json) and import it for existing tenants.
...
The dashboard is created automatically for each new tenant that you add to WinstarCloud.
You can also download the dashboard JSON [here](https://github.com/winstarcloud/winstarcloud/blob/master/application/src/main/data/json/demo/dashboards/software.json) and import it for existing tenants.
