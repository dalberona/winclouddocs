...
{% assign mobilePrefix = "WinstarCloud PE Mobile Application" %}
{% assign mobileUrl = "/docs/pe/mobile/" %}
{% assign mobileGuide = "/docs/pe/mobile/getting-started/" %}
{% else %}
{% assign mobilePrefix = "WinstarCloud Mobile Application" %}
...
The WinstarCloud Notification center is a comprehensive tool for sending, managing, and automating notifications within the platform. It allows for a variety of notification methods, including web, email, mobile application, SMS, and integrations with Slack and Microsoft Teams. 
...
WinstarCloud offers several notification delivery methods to keep you and your customers promptly informed about any events in your IoT solution:

- **Web**. Receive notifications directly within the WinstarCloud web interface. This is perfect for users who are always logged in.
...
- **Slack**. Integrate Slack with WinstarCloud to send notifications as messages to individual users or channels within your Slack workspace. To use this method of notification delivery, you first need to configure the Slack settings in WinstarCloud using [this guide](/docs/{{docsPrefix}}user-guide/ui/slack-settings/).
- **Microsoft Teams**. Integration of Microsoft Teams with WinstarCloud allows for delivering notifications in the form of messages to specific channels in your Microsoft Teams environment. To use this method a tenant administrator must get **webhook URL** for a needed Microsoft Teams channel using this [guide](/docs/{{docsPrefix}}user-guide/ui/microsoft-teams-settings/).
...
For delivering notifications through the WinstarCloud platform, Microsoft Teams, and Slack, you'll additionally need to specify the exact destination for your notification at the recipient adding stage.
...
#### WinstarCloud platform users

To add recipient from the WinstarCloud user list, follow these steps:
...
Send WinstarCloud notifications as Slack messages to a public or private channel, or direct message.

{% capture difference %}
**Please note:**
first, you need to configure the Slack settings in WinstarCloud using [this guide](/docs/{{docsPrefix}}user-guide/ui/slack-settings/).
{% endcapture %}
{% include templates/info-banner.md content=difference %}
...
Send WinstarCloud notifications as messages to your Microsoft Teams channel.
...
The notification in WinstarCloud may look like this:
...
Let's consider an example with the following event: user johndoe@winstarcloud.io added new device 'Compressor AO-99'.
...
Let's consider an example with the following event: johndoe@winstarcloud.io assigned the 'High Temperature' alarm of the device 'Compressor BJ-66' to janesmith@winstarcloud.io.
...
The Edge connection template is used to send notifications about changes in the connection status between WinstarCloud and the Edge.
...
Let's consider an example with the following event: WinstarCloud connected to Edge.
...
The Edge communication failure template is used to send notifications about WinstarCloud connection failures with the Edge.
...
Let's consider an example with the following event: failed to connect WinstarCloud to Edge.
...
This template is intended to notify tenants about the release of a new version of the WinstarCloud platform. Only the system administrator can use this template.
Available template parameters contain all parameters available for the [General](#general) template, plus:
...
The system administrator can use the "New platform version" rule to notify tenants and system administrators about the release of a new version of the WinstarCloud platform.
...
The default rule 'New platform version' will notify affected tenant administrators and system administrators when a new version of the WinstarCloud platform is released.

Template subject: `New version ${latestVersion} is available`

Template message: `Current version is ${currentVersion}. You can upgrade your WinstarCloud instance to version ${latestVersion}.`
