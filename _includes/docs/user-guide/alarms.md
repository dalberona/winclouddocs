...
WinstarCloud provides the ability to create and manage alarms related to your entities: devices, assets, customers, etc.
...
For example, you may configure WinstarCloud to automatically create an alarm when the temperature sensor reading is above a certain threshold.
...
The alarm originator is an entity that causes the alarm. For example, "Device A" is the initiator of an alarm if WinstarCloud receives a temperature reading from it that exceeds the threshold value specified in the alarm rule, and it creates a "High Temperature" alarm.
...
Alarms in WinstarCloud can be **active** or **cleared**. The system can automatically clear an alarm if a predefined condition is met, though setting such an Alarm clear condition is optional. 
Additionally, users have the option to manually clear alarms.

Apart from the active and cleared states, WinstarCloud also monitors whether an alarm has been acknowledged by a user.
...
WinstarCloud identifies alarm using a combination of originator, type, and start time. 
...
The alarm has a start time and a creation time. The alarm start time is determined by the moment the set threshold is exceeded (i.e., when the conditions for triggering the alarm first occurred), while the alarm creation time refers to the moment when the WinstarCloud system received telemetry from the device and processed it.
...
And if the processing of this telemetry reveals that a threshold has been exceeded, WinstarCloud will generate an alarm. The alarm start time will be the timestamp of the message with the threshold value exceeded, and the alarm creation time will be the time WinstarCloud processed messages from the device.
...
- At 12:08, the WinstarCloud receives data from the device, analyzes it, and registers the alarm. The alarm created time is 12:08, as that is the moment when the WinstarCloud system received and processed the device message.
...
When WinstarCloud creates an alarm, in addition to the creation and start time, it also stores the **end time** of the alarm. By default, the start time and the end time are the same. 
...
To simplify the database queries and improve load time, WinstarCloud supports the propagation of the alarm.
...
Let's look at an example of how it works. But first, we recommend reading the "[Getting Started](/docs/{{docsPrefix}}getting-started-guides/helloworld/)" guide. This will improve your understanding of WinstarCloud devices and dashboards and the concepts presented here.
...
To send notifications, including alarms, there is the WinstarCloud [Notification Center](/docs/{{docsPrefix}}user-guide/notifications).
By default, you will receive notifications about all your alarms and alarms of your customers.
The Notification Center allows you to flexibly configure rules for sending notifications about alarms to end users through the WinstarCloud web interface, email, Slack, Microsoft Teams, or SMS.
...
Also, the WinstarCloud has handy widgets for visualizing alarms on the dashboard.
...
WinstarCloud provides REST API to manage and query alarms. See demo environment [Alarm REST API](https://demo.winstarcloud.io/swagger-ui.html#/alarm-controller) and general [REST API](/docs/{{docsPrefix}}reference/rest-api/) documentation for more details.