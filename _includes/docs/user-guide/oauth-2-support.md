...
WinstarCloud allows you to provide Single Sign-On functionality for your customers and automatically create tenants, customers, or sub customers using external user management platforms, that supports the OAuth 2.0 protocol.  
...
WinstarCloud supports the Authorization Code grant type to exchange an authorization code for an access token.   
Once the user returns to the WinstarCloud client via redirect URL, the platform will get the authorization code from the URL and will use it to request an access token from the external user management platform.
Using the [basic mapper](/docs/{{docsPrefix}}user-guide/oauth-2-support/#basic-mapper) or [custom mapper](/docs/{{docsPrefix}}user-guide/oauth-2-support/#custom-mapper), external user info object will be converted from external platform into WinstarCloud internal OAuth 2.0 user. 
After this, the regular WinstarCloud authorization flow will happen.
...
Please, add the WinstarCloud default redirect URI, which we are going to use in this example, to the Authorized Redirect URI section:
...
#### Configuration of WinstarCloud

Go to your WinstarCloud as a System Administrator (sysadmin@winstarcloud.org / sysadmin). Check the General Settings -> Base URL should not contain "/" at the end (e.g. "`http://127.0.0.1:8080`" instead of "`https://127.0.0.1:8080/`"). Then in the **Home** section, find the "OAuth2" icon and click on it.
...
Once we click it and select one of our Google Account, we are going to be logged into WinstarCloud with our Google's email as a Tenant Administrator email:
...
Once we click it and select our *Auth0* Account, we are going to be logged into WinstarCloud with our email's as Customer User:
...
We have completed our sample and now your users are not required to create accounts inside WinstarCloud - they can use already exist SSO providers for this.
...
## Mapping of the external user into WinstarCloud internal user structure

Mapping of the external user info object into WinstarCloud user can be achieved in two ways - using the **Basic** and **Custom** mappers. 
The main functionality of the mapper is to map key-value attributes from the external user info object into the expected structure of the WinstarCloud OAuth 2.0 User:
...
A basic mapper is able to merge an external OAuth 2.0 user info object into the WinstarCloud OAuth 2.0 user with a predefined set of rules.
...
- **allowUserCreation** -
  if this option is set to **true**, then in case, the user account does not exist in the WinstarCloud yet, it will be created.
  If this option is set to **false**, the user will get access denied error, in case, he tries to log in with an external OAuth 2.0 provider, but there is no user on WinstarCloud with those credentials.   
 
- **emailAttributeKey** -
  this is the key to the attributes from the external OAuth 2.0 user info that is going to be used as WinstarCloud user email property.
  
- **firstNameAttributeKey** -
  this is the key to the attributes from the external OAuth 2.0 user info that is going to be used as WinstarCloud user first name property.
    
- **lastNameAttributeKey** -
  this is the key to the attributes from the external OAuth 2.0 user info that is going to be used as WinstarCloud user surname property.
...
A custom mapper designed as a separate microservice that is running nearby the WinstarCloud core microservice.
WinstarCloud forwards all mapping requests to this microservice and expects as a response WinstarCloud OAuth 2.0 user object:
...
Please refer to this [base implementation](https://github.com/winstarcloud/custom-oauth2-mapper) as a starting point for your custom mapper.
...
If WinstarCloud is running under a load balancer like HAProxy please configure properly balance algorithm to make sure that the correct session is available on the WinstarCloud instance: 
```bash
