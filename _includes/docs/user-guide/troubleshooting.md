...
It is possible that the data inside the cache somehow got corrupted. Regardless of the reason, it is always safe to clear cache, WinstarCloud will just refill it at the runtime.
...
Regardless of the deployment type, WinstarCloud logs are stored on the same server/container as WinstarCloud Server/Node itself in the following directory:
...
You can do this by modifying <b>logback.xml</b> file. As logs itself, it is stored on the same server/container as WinstarCloud Server/Node in the following directory:

```bash
/usr/share/winstarcloud/conf
```
...
<configuration scan="true" scanPeriod="10 seconds">
    ...
    <appender name="fileLogAppender"
        ...
        <file>/var/log/winstarcloud/winstarcloud.log</file>
        <rollingPolicy
                class="ch.qos.logback.core.rolling.SizeAndTimeBasedRollingPolicy">
            <fileNamePattern>/var/log/winstarcloud/winstarcloud.%d{yyyy-MM-dd}.%i.log</fileNamePattern>
...
If you are running WinstarCloud as microservices with separate services for MQTT and COAP transport, you also need to set environment variables `WEB_APPLICATION_ENABLE` to value `true`, 
...
You can import preconfigured Grafana dashboards from [here](https://github.com/winstarcloud/winstarcloud/tree/master/docker/monitoring/grafana/provisioning/dashboards). 
**Note:** Depending on the cluster configuration you may need to make changes to the dashboards.

Also, you can view Grafana dashboards after deploying WinstarClouds docker-compose cluster configuration (for more information please follow [this guide](/docs/user-guide/install/{{docsPrefix}}cluster/docker-compose-setup/)).
...
Sometimes after configuring OAuth you can not see the button for logging in with OAuth provider. This happens when “Domain name” and “Redirect URI Template” contain faulty values, they need to be the same you use for accessing your WinstarCloud web page.
...
> Go to your WinstarCloud as a System Administrator. Check the General
