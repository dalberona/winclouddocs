...
Since WinstarCloud 3.2, the Tenant administrator is able to configure common settings for multiple devices using Device Profiles. 
Each Device has one and only profile at a single point in time. 

Experienced WinstarCloud users can notice that the device type has been deprecated in favor of the Device Profile. 
...
To avoid this painful and mundane activity, since WinstarCloud 3.2, you can specify a custom Rule Chain for your devices.
...
- Use the command below to publish time-series data. {% if (docsPrefix == null) or (docsPrefix == "pe/") %}Don't forget to replace `$WINSTARCLOUD_HOST_NAME` with your host.{% endif %}
  {% if (docsPrefix == null) or (docsPrefix == "pe/") %}
```bash
mosquitto_pub -h '$WINSTARCLOUD_HOST_NAME' -i 'c1' -u 't1' -P 'secret' -t '/telemetry' -m '{"humidity": 10.3}'
```
{: .copy-code}
{% endif %}
{% if docsPrefix == "paas/" %}
```bash
mosquitto_pub -h 'mqtt.winstarcloud.cloud' -i 'c1' -u 't1' -P 'secret' -t '/telemetry' -m '{"humidity": 10.3}'
```
{: .copy-code}
{% endif %}
...
{% if (docsPrefix == null) or (docsPrefix == "pe/") %}Don't forget to replace `$WINSTARCLOUD_HOST_NAME` with your host.{% endif %}

- Command for publish timeseries data:
{% if (docsPrefix == null) or (docsPrefix == "pe/") %}
```bash
mosquitto_pub -h '$WINSTARCLOUD_HOST_NAME' -i 'c1' -u 't1' -P 'secret' -t 'v1/devices/me/telemetry' -m '{"humidity": 10.3}'
```
{: .copy-code}
{% endif %}
{% if docsPrefix == "paas/" %}
```bash
mosquitto_pub -h 'mqtt.winstarcloud.cloud' -i 'c1' -u 't1' -P 'secret' -t 'v1/devices/me/telemetry' -m '{"humidity": 10.3}'
```
{: .copy-code}
{% endif %}
...
{% if (docsPrefix == null) or (docsPrefix == "pe/") %}
```bash
mosquitto_pub -h '$WINSTARCLOUD_HOST_NAME' -i 'c1' -u 't1' -P 'secret' -t 'v1/devices/me/attributes' -m '{"firmwareVersion": "1.3"}'
```
{: .copy-code}
{% endif %}
{% if docsPrefix == "paas/" %}
```bash
mosquitto_pub -h 'mqtt.winstarcloud.cloud' -i 'c1' -u 't1' -P 'secret' -t 'v1/devices/me/attributes' -m '{"firmwareVersion": "1.3"}'
```
{: .copy-code}
{% endif %}
...
The current version of the WinstarCloud platform supports customizable proto schemas for [telemetry upload](/docs/{{docsPrefix}}reference/mqtt-api/#telemetry-upload-api) 
...
WinstarCloud parses the protobuf structures dynamically, that is why, it does not support some protobuf features like OneOf, extensions and maps, yet.
...
The current version of the WinstarCloud platform supports customizable proto schemas for [telemetry upload](/docs/{{docsPrefix}}reference/coap-api/#telemetry-upload-api) 
...
The current version of the WinstarCloud platform supports integration with next Efento NB-IoT sensors: 
...
Since WinstarCloud 3.2, we have introduced Alarm Rules to simplify the process of configuring the most popular alarm types.
...
{% if (docsPrefix == null) or (docsPrefix == "pe/") %}
```bash
mosquitto_pub -d -h '$WINSTARCLOUD_HOST_NAME' -t "v1/devices/me/telemetry" -u "$ACCESS_TOKEN" -m '{"temperature": 5.3}'
```
{: .copy-code}
Where:
...
{% if docsPrefix == "paas/" %}
```bash
mosquitto_pub -d -h 'mqtt.winstarcloud.cloud' -t "v1/devices/me/telemetry" -u "$ACCESS_TOKEN" -m '{"temperature": 5.3}'
```
{: .copy-code}
...
Assuming you have configured alarm rules you may also want to receive a notification when WinstarCloud creates or updates the alarm.
...
Device provisioning allows a device to automatically register in WinstarCloud either during or after manufacturing. 
