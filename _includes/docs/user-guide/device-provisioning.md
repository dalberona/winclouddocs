...
As a device manufacturer or firmware developer, I would like my devices to automatically provision themselves in WinstarCloud.
...
From the 3.5 version, WinstarCloud allows the auto-provision of new devices on authentication over MQTT using [X.509 Certificate chain](/docs/{{docsPrefix}}user-guide/certificates/).
   
## How it works?

<object width="80%" data="/images/user-guide/device-provisioning/flow.svg"></object>

The device may send a device provisioning request (Request) to the WinstarCloud. The Request should always contain a Provision key and secret. 
...
The WinstarCloud validates the Request and replies with the device provisioning response (Response).
...
During the validation of the Request, WinstarCloud will first check the supplied *provisionDeviceKey* and *provisionDeviceSecret* to find the corresponding [Device Profile](/docs/{{docsPrefix}}user-guide/device-profiles/).
...
* **Allowing creating new devices** - checks that the device with the same name has not registered in WinstarCloud yet.
...
* **Checking pre-provisioned devices** - checks that the device with the same name has been already created in WinstarCloud, but hasn't been provisioned yet.
...
You can use [bulk provisioning](/docs/{{docsPrefix}}user-guide/bulk-provisioning/) to upload this list to WinstarCloud. Now, devices from the list can issue provision requests, and no other devices will be able to provision themselves.

Once the provisioning is complete, WinstarCloud will update *provisionState* server attribute of the device and will set it to *provisioned* value.
...
Credentials generated by the WinstarCloud server%,%without%,%templates/provisioning/mqtt-without-credentials-type.md%br%
...
Credentials generated by WinstarCloud server%,%without%,%templates/provisioning/http-without-credentials-type.md%br%
...
Credentials generated by WinstarCloud server%,%without%,%templates/provisioning/coap-without-credentials-type.md%br%
