...
In this tutorial, you will learn how to create custom rule nodes and add them to your WinstarCloud server instance.
...
```bash
git clone -b {{ site.release.branch }} https://github.com/winstarcloud/rule-node-examples
cd rule-node-examples
```
{: .copy-code}

By default, sample project is configured to use APIs of the WinstarCloud Community Edition. 
...
you should change the "winstarcloud.version" parameter in winstarcloud.yml:
...
...
    <properties>
        ...
        <winstarcloud.version>{{ site.release.pe_full_ver }}</winstarcloud.version>
        ...
    </properties>
...
```
...
[TbNode](https://github.com/winstarcloud/winstarcloud/blob/{{ site.release.branch }}/rule-engine/rule-engine-api/src/main/java/org/winstarcloud/rule/engine/api/TbNode.java) interface and annotate it with the
[RuleNode](https://github.com/winstarcloud/winstarcloud/blob/{{ site.release.branch }}/rule-engine/rule-engine-api/src/main/java/org/winstarcloud/rule/engine/api/RuleNode.java) annotation.
...
This values should correspond to the ones that are used in [TbContext.tellNext](https://github.com/winstarcloud/winstarcloud/blob/{{ site.release.branch }}/rule-engine/rule-engine-api/src/main/java/org/winstarcloud/rule/engine/api/TbContext.java#L76) method;
* *customRelations* - boolean value that indicates you are going to use any custom relations in [TbContext.tellNext](https://github.com/winstarcloud/winstarcloud/blob/{{ site.release.branch }}/rule-engine/rule-engine-api/src/main/java/org/winstarcloud/rule/engine/api/TbContext.java#L76) method;
...
This method is mostly used to parse the configuration which is a JSON object or to obtain a local copy of [TbContext](https://github.com/winstarcloud/winstarcloud/blob/{{ site.release.branch }}/rule-engine/rule-engine-api/src/main/java/org/winstarcloud/rule/engine/api/TbContext.java).
...
### Using WinstarCloud services

The [TbContext](https://github.com/winstarcloud/winstarcloud/blob/{{ site.release.branch }}/rule-engine/rule-engine-api/src/main/java/org/winstarcloud/rule/engine/api/TbContext.java) contains "getters" for a lot of useful services.
...
WinstarCloud PE users may get access to additional services using *TbContext.getPeContext()* method. TbPeContext provides access to the following services:
...
void changeDashboardOwner(TenantId tenantId, EntityId targetOwnerId, Dashboard dashboard) throws WinstarcloudException;

void changeUserOwner(TenantId tenantId, EntityId targetOwnerId, User user) throws WinstarcloudException;

void changeCustomerOwner(TenantId tenantId, EntityId targetOwnerId, Customer customer) throws WinstarcloudException;

void changeEntityViewOwner(TenantId tenantId, EntityId targetOwnerId, EntityView entityView) throws WinstarcloudException;

void changeAssetOwner(TenantId tenantId, EntityId targetOwnerId, Asset asset) throws WinstarcloudException;

void changeDeviceOwner(TenantId tenantId, EntityId targetOwnerId, Device device) throws WinstarcloudException;

void changeEntityOwner(TenantId tenantId, EntityId targetOwnerId, EntityId entityId, EntityType entityType) throws WinstarcloudException;
...
You may notice that we have used [TbContext.enqueueForTellNext](https://github.com/winstarcloud/winstarcloud/blob/{{ site.release.branch }}/rule-engine/rule-engine-api/src/main/java/org/winstarcloud/rule/engine/api/TbContext.java#L119) method to push new message to the Rule Engine.
...
[TbNode.onMsg](https://github.com/winstarcloud/winstarcloud/blob/{{ site.release.branch }}/rule-engine/rule-engine-api/src/main/java/org/winstarcloud/rule/engine/api/TbNode.java#L30) method sequentially
...
As a rule node developer, you may override default method [TbNode.onPartitionChangeMsg](https://github.com/winstarcloud/winstarcloud/blob/{{ site.release.branch }}/rule-engine/rule-engine-api/src/main/java/org/winstarcloud/rule/engine/api/TbNode.java#L34)
...
In order to determine that the current entity id belongs to current list of assigned partitions, one may use [TbContext.isLocalEntity](https://github.com/winstarcloud/winstarcloud/blob/{{ site.release.branch }}/rule-engine/rule-engine-api/src/main/java/org/winstarcloud/rule/engine/api/TbContext.java#L152).
See complete example below:

```java
package org.winstarcloud.rule.engine.node.filter;
...
## Step 4. Import custom rule nodes to your WinstarCloud instance
...
Then, locate the jar-file to your WinstarCloud project as dependency library. The result of the build is located here:

```bash
target/rule-engine-1.0.0-custom-nodes.jar
```

Now you are ready to add the jar-file with your rule nodes to your WinstarCloud instance:
{% unless docsPrefix %}
* Use Step 4.1 if your WinstarCloud is installed as a service. 
* Use Step 4.2 if your WinstarCloud is built from sources and launched locally from your IDE

### Step 4.1 Add JAR file to WinstarCloud installed as a service
{% endunless %}

 - first, you need to execute the following command to copy jar-file to WinstarCloud extensions:

```bash
sudo cp rule-engine-1.0.0-custom-nodes.jar /usr/share/winstarcloud/extensions/
```

 - next, execute the following to change the owner to WinstarCloud:

```bash
sudo chown winstarcloud:winstarcloud /usr/share/winstarcloud/extensions/*
```

Restart Winstarcloud service:

```bash
sudo service winstarcloud restart
```

**Once WinstarCloud was restarted you need to clear browser cache and refresh the web page to reload UI of Rule Nodes**

{% unless docsPrefix %}
### Step 4.2 Add JAR file to local WinstarCloud launched using IDE
...
Restart WinstarCloud server-side container. Please, refer to the following link to see how to do this: [Running server-side container](/docs/{{docsPrefix}}user-guide/contribution/how-to-contribute/#running-server-side-container). 
 
**Once WinstarCloud was restarted you need to clear browser cache and refresh the web page to reload UI of Rule Nodes**
{% endunless %}

## Step 5. Add your custom package name to winstarcloud.yml

**NOTE** if you have changed the package name from **org.winstarcloud.rule.engine** to your company package name, e.g. **com.example.rule.engine**, 
you need also to add your package name in **winstarcloud.yml** file in plugins section:

```yaml
# Plugins configuration parameters
plugins:
  # Comma separated package list used during classpath scanning for plugins
  scan_packages: "${PLUGINS_SCAN_PACKAGES:org.winstarcloud.server.extensions,org.winstarcloud.rule.engine,com.example.rule.engine}"
...
The WinstarCloud rule nodes UI is configured with another project in the official [github repo](https://github.com/winstarcloud/rule-node-examples-ui-ngx). Please, refer to the following [link](https://github.com/winstarcloud/rule-node-examples-ui-ngx#rule-node-examples-ui-ngx) to see build instructions.
