...
This is a widget development guide for the WinstarCloud version before the WinstarCloud 3.0 version. 
In case if you are using the WinstarCloud 3.0+ version, you can refer to the next link for the [Widget Development WinstarCloud 3.0+ version](/docs/{{docsPrefix}}user-guide/contribution/widgets-development/) guide.

**WinstarCloud widgets** are additional UI modules that can be easily integrated into any [IoT Dashboards](/docs/{{docsPrefix}}user-guide/dashboards/) and provide end-user functions such as data visualization, remote device control, alarms management and displaying static custom html content.
...
It has mostly all functionality provided by usual WinstarCloud dashboard but has some limitations.
...
The function **getAlarmValue** is fetching alarm value using special alarmFields constants obtained from **types** which is part of WinstarCloud UI and accessed via Angular **$injector**.
...
In this case, you can create own JavaScript class or Angular directive and bundle it into the WinstarCloud UI code.
...
Some of the WinstarCloud widgets already use this approach. Take a look at the [widget.service.js](https://github.com/winstarcloud/winstarcloud/blob/v2.5.5/ui/src/app/api/widget.service.js).
Here you can find how some bundled classes or modules are registered for later use in WinstarCloud widgets.
For example "Timeseries - Flot" widget (from "Charts" Widgets Bundle) uses [**TbFlot**](https://github.com/winstarcloud/winstarcloud/blob/v2.5.5/ui/src/app/widget/lib/flot-widget.js) JavaScript class which is injected as window property inside **widget.service.js**:
...
Another example is "Timeseries table" widget (from "Cards" Widgets Bundle) that uses Angular directive [**tb-timeseries-table-widget**](https://github.com/winstarcloud/winstarcloud/blob/v2.5.5/ui/src/app/widget/lib/timeseries-table-widget.js) which is registered as dependency of **'winstarcloud.api.widget'** Angular module inside **widget.service.js**.
Thereby this directive becomes available for use inside the widget template HTML. 

```javascript
...

import winstarcloudTimeseriesTableWidget from '../widget/lib/timeseries-table-widget';

...

export default angular.module('winstarcloud.api.widget', ['oc.lazyLoad', winstarcloudLedLight, winstarcloudTimeseriesTableWidget,
