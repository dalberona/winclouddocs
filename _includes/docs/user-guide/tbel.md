...
WinstarCloud supports user-defined functions (UDF) for data processing in the [Rule Engine]() and [Data Converters](). 
...
WinstarCloud is written in Java and currently uses Java 11. There are two ways to execute the JS function in WinstarCloud:  

A) use remote JS Executor microservice written in Node.js. It is the default way for WinstarCloud to run in a cluster/microservices mode;

B) use local JS Executor powered by Nashorn JS engine that runs inside the JVM. It is the default way for WinstarCloud while deployed in monolithic mode.
...
The WinstarCloud Expression Language (TBEL) is basically a [fork](https://github.com/winstarcloud/tbel) of MVEL with some important security constraints, 
built-in memory management, and frequently used helper functions that are specific to WinstarCloud.
...
Assuming you are using TBEL in the Rule Engine to define a simple script [filter node](https://winstarcloud.io/docs/user-guide/rule-engine-2-0/filter-nodes/#script-filter-node):
