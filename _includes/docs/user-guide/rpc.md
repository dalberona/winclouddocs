...
WinstarCloud allows you to send Remote Procedure Calls (RPC) from server-side applications to devices and vice versa.
Basically, this feature allows you to send commands to/from devices and receive results of commands execution. 
This guide covers WinstarCloud RPC capabilities. After reading this guide, you will get familiar with the following topics:
...
WinstarCloud RPC feature may be divided into two types based on the originator of the remote procedure execution:device-originated and server-originated RPC.
...
WinstarCloud provides an API to send RPC commands from the device.
...
Before version 3.3, WinstarCloud supported **lightweight** RPC only. 
...
They lived in memory of the server, assuming that if server dies, the dashboard widget will send the same request to other WinstarCloud server in the cluster.
...
Since version 3.3, WinstarCloud provides support of [**persistent**](#persistent-rpc) RPC calls.
...
- **http(s)://host:port** is your WinstarCloud server base URL. For example, [https://winstarcloud.cloud](https://winstarcloud.cloud)
...
WinstarCloud provides a convenient API to receive and process server-side RPC commands on the device.
...
WinstarCloud tracks state of the persistent RPC. There are 7 available states:

{% if docsPrefix == 'paas/' %}

* **QUEUED** - RPC was created and saved to the database; 
  No attempt to send the RPC to device yet; 
  WinstarCloud will attempt to send the RPC immediately when device becomes online or if it is already online;
...
* **SENT** - WinstarCloud performed attempt to send the RPC to device.
* **DELIVERED** - device confirmed that the RPC was delivered; This is the last step of processing for **one-way** RPC;   
* **SUCCESSFUL** - WinstarCloud received reply for the **two-way** RPC;  
* **TIMEOUT** - WinstarCloud transport layer (MQTT/CoAP/LwM2M, etc) detected timeout of the RPC delivery;
