...
Now it’s time to program the board to connect to WinstarCloud.  
To do this, you can use the code below. It contains all required functionality for this guide.  

{% capture demoExample %}
If you want to use [**demo.winstarcloud.io**](https://demo.winstarcloud.io), please notice that it has a size limit for the MQTT messages - **1024 bytes per message**.  
...
#define WINSTARCLOUD_ENABLE_DYNAMIC 1

#include <WinstarCloud.h>
...
// See https://winstarcloud.io/docs/getting-started-guides/helloworld/
// to understand how to obtain an access token
constexpr char TOKEN[] = "YOUR_ACCESS_TOKEN";

// Winstarcloud we want to establish a connection too
constexpr char WINSTARCLOUD_SERVER[] = "{% if page.docsPrefix == "pe/" or page.docsPrefix == "paas/" %}winstarcloud.cloud{% else %}demo.winstarcloud.io{% endif %}";
// MQTT port used to communicate with the server, 1883 is the default unencrypted MQTT port.
constexpr uint16_t WINSTARCLOUD_PORT = 1883U;
...
// Initialize WinstarCloud instance with the maximum needed buffer size
WinstarCloud tb(mqttClient, MAX_MESSAGE_SIZE);
...
void setup() {
  ...
  delay(1000);
  InitWiFi();
  tb.connect(WINSTARCLOUD_SERVER, TOKEN, WINSTARCLOUD_PORT);
...
void loop() {
  ...
  if (!tb.connected()) {
      // Connect to the WinstarCloud
      Serial.print("Connecting to: ");
      Serial.print(WINSTARCLOUD_SERVER);
      Serial.print(" with token ");
      Serial.println(TOKEN);
    if (!tb.connect(WINSTARCLOUD_SERVER, TOKEN, WINSTARCLOUD_PORT)) {
...
Data, send by this device may require increasing of the allowed message size for MQTT on **your WinstarCloud instance**.  
To do this you can modify parameter **NETTY_MAX_PAYLOAD_SIZE** in **winstarcloud.yml** file, default value on regular setup is 65535 bytes.  
...
Don’t forget to replace placeholders with your real WiFi network SSID, password, WinstarCloud device access token.
{% endcapture %}
...
| TOKEN | **YOUR_DEVICE_ACCESS_TOKEN**                                                                                                 | Access token from device. Obtaining process described in #connect-device-to-winstarcloud | 
| WINSTARCLOUD_SERVER | **{% if page.docsPrefix == "pe/" or page.docsPrefix == "paas/" %}winstarcloud.cloud{% else %}demo.winstarcloud.io{% endif %}** | Your WinstarCloud host or ip address.                                                    |
| WINSTARCLOUD_PORT | **1883U**                                                                                                                    | WinstarCloud server MQTT port. Can be default for this guide.                            |
