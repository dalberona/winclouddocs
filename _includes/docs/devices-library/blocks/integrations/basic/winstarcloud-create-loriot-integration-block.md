...
Now we need to copy the "**Application ID**" in the Loriot. It is required for configuring the integration in WinstarCloud. 
...
Next we will create an integration with Loriot inside the WinstarCloud.  
...
{% assign loriotTroubleshooting = '
    ===
        ...
        title: Go to edit mode WinstarCloud Loriot integration, switch off **"Create Loriot Application output"** slider and copy **"HTTP endpoint URL"**. Then, apply changes;
    ===
