...
WinstarCloud rule engine supports basic analysis of incoming telemetry data, for example, threshold crossing.
...
This tutorial will demonstrate how you can:
 ...
 - route telemetry device data from WinstarCloud to Kafka topic using the built-in rule engine capabilities (works for both WinstarCloud CE and PE).
 - aggregate data from multiple devices using a simple Kafka Streams application.
 - push results of the analytics back to WinstarCloud for persistence and visualization using WinstarCloud PE Kafka Integration.
...
WinstarCloud is used to collect, store and visualize anomaly telemetry from these solar modules in each panels.
...
* WinstarCloud PE v2.4.2+ [instance](/docs/user-guide/install/pe/installation-options/)
...
Let's create three devices with type "solar-module". If you are using WinstarCloud PE, you cna put them to new "Solar Modules" group.
...
WinstarCloud subscribed to this topic using Kafka Integration, generate alarms and store anomalies to the database.


#### Download the sample application

Feel free to grab the [code from the WinstarCloud repository](https://github.com/winstarcloud/kafka-streams-example) and build the project with maven:
...
The Kafka Streams Application logic is concentrated mainly in the [SolarConsumer](https://github.com/winstarcloud/kafka-streams-example/blob/master/src/main/java/org/winstarcloud/kafka/SolarConsumer.java) class.
...
Let's configure WinstarCloud to subscribe to the “solar-module-anomalies” topic and create alarms. We will use Kafka Integration that is available since WinstarCloud v2.4.2.
