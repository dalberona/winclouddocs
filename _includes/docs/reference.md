...
* **durable**: never lose your data. WinstarCloud supports various queue implementations to provide extremely high message durability.
...
**WinstarCloud Transports**
 
WinstarCloud provides [MQTT](/docs/{{docsPrefix}}reference/mqtt-api/), [HTTP](/docs/{{docsPrefix}}reference/http-api/), [CoAP](/docs/{{docsPrefix}}reference/coap-api/) and [LwM2M](/docs/{{docsPrefix}}reference/lwm2m-api/) based APIs that are available for your device applications/firmware. 
...
WinstarCloud Core is responsible for handling [REST API](/docs/{{docsPrefix}}reference/rest-api/) calls and WebSocket [subscriptions](/docs/{{docsPrefix}}user-guide/telemetry/#websocket-api).
...
WinstarCloud Core uses Actor System under the hood to implement actors for main entities: tenants and devices. 
Platform nodes can join the cluster, where each node is responsible for certain partitions of the incoming messages.

**WinstarCloud Rule Engine**

WinstarCloud Rule Engine is the heart of the system and is responsible for processing incoming [messages](/docs/{{docsPrefix}}user-guide/rule-engine-2-0/overview/#rule-engine-message).
...
WinstarCloud Rule Engine may operate in two modes: shared and isolated. In shared mode, rule engine process messages that belong to multiple tenants.
In isolated mode Rule Engine may be configured to process messages for tenants of specific tenant profile only. 

**WinstarCloud Web UI**

WinstarCloud provides a lightweight component written using Express.js framework to host static web ui content. 
Those components are completely stateless and no much configuration available. 
The static web UI contains application bundle. Once it is loaded, the application starts using the REST API and WebSockets API provided by WinstarCloud Core.  
 
## Message Queues are awesome!

WinstarCloud supports multiple message queue implementations: Kafka, RabbitMQ, AWS SQS, Azure Service Bus and Google Pub/Sub. We plan to extend this list in the future.
Using durable and scalable queues allow WinstarCloud to implement back-pressure and load balancing. Back-pressure is extremely important in case of peak loads.  
...
WinstarCloud message Producers determines which partition to use based on the hash of entity id. 
Thus, all messages for the same entity are always pushed to the same partition.
WinstarCloud message Consumers coordinate using Zookeeper and use consistent-hash algorithm to determine list of partitions that each Consumer should subscribe to.
...
WinstarCloud uses following topics:

 * **tb_transport.api.requests**: to send generic API calls to check device credentials from Transport to WinstarCloud Core.
 * **tb_transport.api.responses**: to receive device credentials verification results from WinstarCloud Core to Transport.
 * **tb_core**: to push messages from Transport or Rule Engine to WinstarCloud Core. Messages include session lifecycle events, attribute and RPC subscriptions, etc.
 * **tb_rule_engine**: to push messages from Transport or WinstarCloud Core to Rule Engine. Messages include incoming telemetry, device states, entity lifecycle events, etc.
 
**Note:** All topic properties including names and number of partitions are [configurable](/docs/user-guide/install/{{docsPrefix}}config/) via winstarcloud.yml or environment variables. 
Since WinstarCloud 3.4 we can configure Rule Engine queues by the UI, see the [documentation](/docs/{{docsPrefix}}user-guide/rule-engine-2-5/queues/).
...
for all communication between WinstarCloud components. 
...
WinstarCloud supports both on-premise and cloud deployments. 
With more then 5000 WinstarCloud servers running all over the world, WinstarCloud is running in production on AWS, Azure, GCE and private data centers.
It is possible to launch WinstarCloud in the private network with no internet access at all.

## Standalone vs cluster mode

Platform is designed to be horizontally scalable and supports automatic discovery of new WinstarCloud servers (nodes). 
All WinstarCloud nodes inside cluster are identical and are sharing the load. 
Since all nodes are identical there is no "master" or "coordinator" processes and there is no single point of failure. 
The load balancer of your choice may forward request from devices, applications and users to all WinstarCloud nodes.

## Monolithic vs microservices architecture

Starting WinstarCloud v2.2, it is possible to run the platform as a monolithic application or as a set of microservices. 
...
* [**monolithic**](/docs/{{docsPrefix}}reference/monolithic): Learn more about deployment, configuring and running WinstarCloud platform in a monolythic mode.  
* [**microservices**](/docs/{{docsPrefix}}reference/msa): Learn more about deployment, configuring and running WinstarCloud platform in a microservices mode.
 

## SQL vs NoSQL vs Hybrid database approach

WinstarCloud uses database to store 
...
* **SQL** - Stores all entities and telemetry in SQL database. WinstarCloud authors recommend to use PostgreSQL and this is the main SQL database that WinstarCloud supports. 
...
* **NoSQL (Deprecated)** - Stores all entities and telemetry in NoSQL database. WinstarCloud authors recommend to use Cassandra and this is the only NoSQL database that WinstarCloud supports at the moment.
...
It is possible to configure this options using **winstarcloud.yml** file. See database [configuration](/docs/user-guide/install/{{docsPrefix}}config/) page for more details.
...
WinstarCloud back-end is written in Java, but we also have some micro-services based on Node.js. WinstarCloud front-end is a SPA based on Angular 9 framework. 
