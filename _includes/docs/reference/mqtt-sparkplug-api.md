...
to get familiar with WinstarCloud. We also recommend to review the [Device Profiles](/docs/{{docsPrefix}}user-guide/device-profiles/) documentation first.
...
WinstarCloud acts as an MQTT Server which support the SparkPlug payload and topic structure and allows connections from the 
...
WinstarCloud decodes the device metrics from the Sparkplug payload and stores it as a corresponding device 
...
[rpc command](#update-metrics--using-the-winstarcloud-rpc-command-from-server-to-mqtt-eondevice).

{% capture difference %}
**NOTE:**
<br>
WinstarCloud supports **Sparkplug™ B** payloads only.
{% endcapture %}
{% include templates/info-banner.md content=difference %}

## Getting started

This guide will teach us how to: connect Sparkplug EoN node to WinstarCloud,
collect device metrics and store them as WinstarCloud time-series data,
...
We have prepared sparkplug node [emulator](https://github.com/winstarcloud/sparkplug-emulator) for the testing purposes.
Let's launch it and connect to our platform instance. We will use access token credentials from the previous step:

{% if docsPrefix == null %}
```bash
docker run -e SPARKPLUG_SERVER_URL='tcp://demo.winstarcloud.io:1883' -e SPARKPLUG_CLIENT_MQTT_USERNAME='YOUR_WINSTARCLOUD_DEVICE_TOKEN' winstarcloud/tb-sparkplug-emulator:latest
```
{: .copy-code}

Don't forget to replace <code>YOUR_WINSTARCLOUD_DEVICE_TOKEN</code> with the actual value of the token.
You should also replace <code>demo.winstarcloud.io</code> with your server hostname.
{% endif %}
{% if docsPrefix == "pe/" %}
```bash
docker run -e SPARKPLUG_SERVER_URL='tcp://YOUR_SERVER_HOSTNAME:1883' -e SPARKPLUG_CLIENT_MQTT_USERNAME='YOUR_WINSTARCLOUD_DEVICE_TOKEN' winstarcloud/tb-sparkplug-emulator:latest
```
{: .copy-code}
...
{% if docsPrefix == "paas/" %}
```bash
docker run -e SPARKPLUG_SERVER_URL='tcp://winstarcloud.cloud:1883' -e SPARKPLUG_CLIENT_MQTT_USERNAME='YOUR_WINSTARCLOUD_DEVICE_TOKEN' winstarcloud/tb-sparkplug-emulator:latest
```
{: .copy-code}

Don't forget to replace <code>YOUR_WINSTARCLOUD_DEVICE_TOKEN</code> with the actual value of the token.
You should also replace <code>winstarcloud.cloud</code> with your server hostname.
{% endif %}
...
### Step 5. Push updates to Sparkplug metrics from Winstarcloud server to MQTT EON and Device

You may push update to Sparkplug node/device metric from WinstarCloud via shared attribute update or RPC command. 

#### Update Metrics using shared attributes

WinstarCloud [Shared Attributes](/docs/{{docsPrefix}}user-guide/attributes/#shared-attributes) are used to deliver metric value updates to the device.
...
#### Update Metrics  using the WinstarCloud RPC command from server to MQTT EON/Device

WinstarCloud supports on-demand update to metrics of the Sparkplug EoN Node or Device using RPC(Remote Procedure Call) feature. We also use term "command" instead of RPC for simplicity.
...
{% include images-gallery.html imageCollection="sparkplug-update-metrics-using-the-winstarcloud-rpc-command-1" %}
...
{% include images-gallery.html imageCollection="sparkplug-update-metrics-using-the-winstarcloud-rpc-command-2" %}
...
{% include images-gallery.html imageCollection="sparkplug-update-metrics-using-the-winstarcloud-rpc-command-3" %}
