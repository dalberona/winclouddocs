...
We assume you have completed the general [Getting Started](/docs/{{docsPrefix}}getting-started-guides/helloworld/) guide to get familiar with WinstarCloud.
...
This part of documentation covers provisioning of your first LwM2M device in WinstarCloud. We will use [Eclipse Wakaama](https://github.com/eclipse/wakaama#test-client-example) test client to simulate LwM2M device.
...
You may notice that Device Object supports Manufacturer, model, and serial numbers. Let’s configure WinstarCloud to fetch
those data when device connects and store it as WinstarCloud attributes.
Also we want to observe Radio Signal Strength, Link Quality and device location push it as WinstarCloud telemetry.
...
WinstarCloud supports 4 different types of credentials: Pre-Shared Key (PSK), Raw Public Ket (RPK), X.509 Certificates and "No Security" mode.
...
```bash
./lwm2mclient -h lwm2m.winstarcloud.cloud -n UniqueEndpointName -p 5685 -c
```
{: .copy-code}

Where
* 'lwm2m.winstarcloud.cloud' is the host name of the LwM2M server;
...
## WinstarCloud LwM2M support

WinstarCloud implements both LwM2M server and bootstrap server that supports plain UDP and DTLS (secure transport over UDP).
As a platform user, you are able to provision and define the mapping between the LwM2M resources and WinstarCloud device [attributes](/docs/{{docsPrefix}}user-guide/attributes/) and [time-series data](/docs/{{docsPrefix}}user-guide/telemetry/).
...
#### Read LwM2M resources to WinstarCloud attributes
...
To do this, select the "Attribute" checkbox for the corresponding Resource. You may optionally change the auto-generated key name, defining the attribute name in WinstarCloud.
For example, let's configure the platform to read the LwM2M Resource */3/0/2* (Device Serial Number) and store it as an attribute *serialNumber* in WinstarCloud:

![image](/images/lwm2m/attributes-ce.png)

WinstarCloud will read the attribute value during device registration (LwM2M "Register" operation) or during registration update (LwM2M "Update" operation).

**Note:** 

We may optionally issue the "Observe" operation to make sure we subscribe to the updates of the LwM2M resource.
To do this, select the "Observe" checkbox for the corresponding Resource. Let's configure WinstarCloud to monitor the LwM2M Resource */3/0/15* (Timezone) and store it as the *timezone* attribute:
...
#### Write LwM2M resource via WinstarCloud attributes update

WinstarCloud [Shared Attributes](/docs/{{docsPrefix}}user-guide/attributes/#shared-attributes) are used to deliver the configuration updates to the device.
You may change the shared attribute in multiple ways - via administration UI, dashboard widget, REST API, or rule engine node. 
Once you change the shared attribute, WinstarCloud will search for the mapping between the attribute key and LwM2M resource.
If the resource is marked as an attribute, platform will send the LwM2M Write operation to the LwM2M client device.

See the *Timezone* example from the [read attributes](#read-lwm2m-resources-to-winstarcloud-attributes)
...
To do this, select the "Telemetry" checkbox for the corresponding Resource. You may optionally change the auto-generated key name, defining the telemetry key in WinstarCloud.
...
and to store them as time-series data in WinstarCloud:

![image](/images/lwm2m/time-series-ce.png)

#### Execute LwM2M operation using the WinstarCloud RPC command

WinstarCloud supports on-demand LwM2M operations using RPC(Remote Procedure Call) feature. We also use "command" to device instead of RPC for simplicity. 
...
* [ObserveCancelAll](#cancel-all-observations-operation) - Winstarcloud-specific operation and allows to cancel all observations on the device at once;
* [ObserveReadAll](#read-all-observations-operation) - Winstarcloud-specific operation and allows to get all observations that are set on the device;
* [DiscoverAll](#discover-all-operation) - Winstarcloud-specific operation and allows to get the object and resources hierarchy, instantiated on the client.
...
So, the following RPC command need to be sent to WinstarCloud:
...
Below you can find examples of usage for commands that are supported by the Winstarcloud platform for LWM2M protocol. Please note that your target client may not support all of them, please refer to the client’s documentation for detailed information on supported commands.
...
# Response:
{"result":"CONTENT","value":"LwM2mObjectInstance [id=0, resources={0=LwM2mSingleResource [id=0, value=Winstarcloud Test Device, 
type=STRING], 1=LwM2mSingleResource [id=1, value=Model 500, type=STRING], 2=LwM2mSingleResource [id=2, value=TH-500-000-0001, 
type=STRING], 3=LwM2mSingleResource [id=3, value=TestWinstarcloud@TestMore1024_2.04, type=STRING], 6=LwM2mSingleResource [id=6, 
...
# Response:
{"result":"CONTENT","value":"{/3/0=LwM2mObjectInstance [id=0, resources={0=LwM2mSingleResource [id=0, value=Winstarcloud Test Device, 
...
3=LwM2mSingleResource [id=3, value=TestWinstarcloud@TestMore1024_2.04, type=STRING], 6=LwM2mSingleResource [id=6, value=1, type=INTEGE
...
# Response:
{"result":"CONTENT","value":"LwM2mObjectInstance [id=0, resources={0=LwM2mSingleResource [id=0, value=Winstarcloud 
Test Device, type=STRING], 1=LwM2mSingleResource [id=1, value=Model 500, type=STRING], 2=LwM2mSingleResource [id=2, 
value=TH-500-000-0001, type=STRING], 3=LwM2mSingleResource [id=3, value=TestWinstarcloud@TestMore1024_2.04, type=STRING], 
...
The "Cancel All Observations" operation is Winstarcloud-specific operation and allows to cancel all observations
...
The "Read All Observations" operation is Winstarcloud-specific operation and allows to get all observations 
...
The "Discover All Observations" operation is Winstarcloud-specific operation and allows to get the object and resources hierarchy,
...
example easing data management. Winstarcloud supports Object Version attribute, which indicates the version of the 
...
The Winstarcloud platform supports secured connection using DTLS. DTLS, which stands for Datagram Transport Layer
Security, is based on the Transport Layer Security (TLS) protocol and built on top of the User Datagram Protocol (UDP).
Winstarcloud allows the use of DTLS  with the LwM2M transport connection for devices.
...
There are three authentication methods available on the Winstarcloud for LwM2M DTLS: using the Pre-Shared Key(PSK), using
the Raw Public Key(RPK) and using the X.509 certificate.

To use DTLS, the end-user device has to connect to the WinstarCloud server using secured port 5686.
...
To configure the connection, you need to do following steps:
   ...
   * Generate Client keys and copy-paste the Client Public key to the Device Credentials - Client Key on the Winstarcloud platform.
...
2. Winstarcloud keeps server keys in the key-storage file "lwm2mserver.jks", please find and copy this file to our folder.
...
#!/bin/bash
#
# Copyright © 2016-2021 The Winstarcloud Authors
#
...
echo "Client public key in base64 format. Copy this key to the Winstarcloud - Client Key field in Device Credentials"
base64 cpubk.der
...
please also update it in ["winstarcloud.yml"](https://winstarcloud.io/docs/user-guide/install/config/) configuration file:
...
{:start="5"}
5. Configure Device on the Winstarcloud platform:
