...
Since WinstarCloud v2.2, the platform supports microservices deployment mode. 
...
Each of the transport servers listed above communicates with the main WinstarCloud Node microservices using Kafka. 
...
with the messages definition available [here](https://github.com/winstarcloud/winstarcloud/blob/master/common/proto/src/main/proto/transport.proto).

**Note**: Starting v2.5, WinstarCloud PE is going to support alternative queue implementation: Amazon DynamoDB. See [roadmap](/docs/{{docsPrefix}}reference/roadmap) for more details.
...
WinstarCloud provides a lightweight component written using Express.js framework to host static web ui content. Those components are completely stateless and no much configuration available. 

## JavaScript Executor Microservices

WinstarCloud rule engine allows users to specify custom javascript functions to parse, filter and transform messages. 
Since those functions are user defined, we need to execute them in an isolated context to avoid impact on main processing.
WinstarCloud provides a lightweight component written using Node.js to execute user defined JavaScript functions remotely to isolate them from the core rule engine components.

**Note**: WinstarCloud monolith app executes user defined functions in a java embedded JS engine, which does not allow to isolate resource consumption.    
...
Each WinstarCloud core service has individual blacklist for JS functions and will not invoke blocked function more then 3(by default) times.

## WinstarCloud Node

WinstarCloud node is a core service written in Java that is responsible for handling:
...
**Note**: moving rule engine to a separate microservice is scheduled for WinstarCloud v2.5. See [roadmap](/docs/{{docsPrefix}}reference/roadmap) for more details. 
...
[Apache Kafka](https://kafka.apache.org/) is an open-source stream-processing software platform. WinstarCloud uses Kafka to persist incoming telemetry from HTTP/MQTT/CoAP transpots 
until it is processed by the rule engine. WinstarCloud also uses Kafka for some API calls between micro-services.

### Redis

[Redis](https://redis.io/) is an open source (BSD licensed), in-memory data structure store used by WinstarCloud for caching. 
WinstarCloud caches assets, entity views, devices, device credentials, device sessions and entity relations.

### Zookeeper

[Zookeeper](https://zookeeper.apache.org/) is an open-source server which enables highly reliable distributed coordination. 
WinstarCloud uses Zookeeper to address requests processing from a single entity (device,asset,tenant) to a certain WinstarCloud server 
...
You can find the reference [haproxy.cfg](https://github.com/winstarcloud/winstarcloud/blob/release-2.5/docker/haproxy/config/haproxy.cfg) 
...
You can find the reference [docker-compose.yml](https://github.com/winstarcloud/winstarcloud/blob/release-2.5/docker/docker-compose.yml)
and corresponding [documentation](https://github.com/winstarcloud/winstarcloud/blob/master/docker/README.md) that will help you to run WinstarCloud containers in a cluster mode 
...
services:
  ...
  tb1:
    ...
    volumes:
      - ./tb-node/conf:/config
      - ./tb-node/log:/var/log/winstarcloud
    depends_on:
      - kafka
      - redis
      - tb-js-executor
