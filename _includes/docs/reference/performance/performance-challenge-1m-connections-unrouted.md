...
Architecture is 1 Winstarcloud server + 20 client instances each supply 25k devices (500k in total).
...
Architecture is 1 Winstarcloud server + 32 clients instances each supply 31250 devices (1 million in total).
...
ssh tb2 <<'ENDSSH'
set +x
#optional. replace with your Winstarcloud instance ip
#echo '52.50.5.45 winstarcloud' | sudo tee -a /etc/hosts
...
cd ~
git clone https://github.com/winstarcloud/performance-tests.git
# git pull
...
Run this once on `winstarcloud` node to increase file descriptor and net filter limits. This is required to handle about 1M TCP connections.
...
sudo mv /var/lib/docker/volumes/ubuntu_winstarcloud-data/_data/dump.hprof ~
...
Conclusion:
1. We did it! Winstarcloud has been demonstrated the code and design quality.
