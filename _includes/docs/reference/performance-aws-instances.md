...
WinstarCloud has been run in production by numerous companies in both [monolithic](/docs/{{docsPrefix}}reference/monolithic/) 
and [microservices](/docs/{{docsPrefix}}reference/msa/) deployment modes.
This article describes the performance of a single WinstarCloud server in the most popular usage scenarios. 
It is helpful to understand how WinstarCloud scales vertically (monolith) before describing how it scales horizontally (cluster mode).   

## Test methodology

For simplicity, we have deployed a single WinstarCloud instance with all related third-party components in a docker-compose environment on a single EC2 instance.
...
WinstarCloud stored all the time-series data in the database.
WinstarCloud also processed the data using the [alarm rules](/docs/{{docsPrefix}}user-guide/device-profiles/#alarm-rules) to create alarms if the battery level is low.
...
[WinstarCloud](/docs/reference/performance/tools/winstarcloud-performance-charts/) used to visualize the performance.

As an output we will analyse the WinstarCloud rule engine [statistics](/docs/{{docsPrefix}}user-guide/rule-engine-2-0/overview/#rule-engine-statistics) dashboard and fancy API usage stats.
...
<details markdown="1">
<summary>
Setup the WinstarCloud instance on AWS EC2
</summary>

Use the Docker Compose file listed below to setup the AWS EC2 instance based on the [instruction](https://github.com/winstarcloud/performance-tests).  
...
services:
  ...
  tb:
    depends_on:
      - postgres
    image: "winstarcloud/tb"
    network_mode: "host"
    restart: "always"
    volumes:
      - winstarcloud-data:/data
      - winstarcloud-logs:/var/log/winstarcloud
    environment:
      ...
      # Postgres connection
      SPRING_DATASOURCE_URL: "jdbc:postgresql://localhost:5432/winstarcloud"
...
Use the Docker command listed below to launch the performance test tool based on the [instruction](https://github.com/winstarcloud/performance-tests).
...
Previous section demonstrates **successful** WinstarCloud deployment designed up to 3000 data points/sec.  
...
WinstarCloud docker compose with no change with the previous section. 
...
Kafka CPU and disk IO overhead are tiny relative to Postgres and WinstarCloud CPU consumption. 
...
Here is the docker-compose file to set up _WinstarCloud + Postgresql + Zookeeper + Kafka_ on AWS EC2 instance based on the [instruction](https://github.com/winstarcloud/performance-tests).
...
WinstarCloud + Postgres + **Kafka** - is a reliable solution to survive peak loads.
...
In a real production, the WinstarCloud may serve all kind of user requests, run custom rule chains and supply the web services for all fancy dashboards.
...
All systems have to run maintenance in background, so it is completely fine to have those chart for WinstarCloud monolith deployment.    
...
Finally, let's check the JVM state on each Winstarcloud, Zookeeper, Kafka and Cassandra
Let's forward JMX port with SSH to connect and monitor all Java applications presented.

```bash
ssh -L 9999:127.0.0.1:9999 -L 1099:127.0.0.1:1099 -L 9199:127.0.0.1:9199 -L 7199:127.0.0.1:7199 winstarcloud 
```
{: .copy-code}
...
Here the JMX monitoring for WinstarCloud, Kafka, Zookeeper, Cassandra. The system is stable.

{% include images-gallery.html imageCollection="cassandra-25k-10k-30k-jmx" %}

**Lessons learned**

**Cassandra** is essential for massive telemetry flow. Peak load is handled with Kafka queue. Bundling Kafka + PostgreSQL + Cassandra is a **top WinstarCloud deployment**. 
...
It is a good idea to start with Cassandra from the very beginning of your WinstarCloud instance and maintain the same stack for the entire project lifetime.
...
System can be scaled up vertically up to 50-100%. For significant horizontal scaling, please, consider to set up a WinstarCloud cluster.
...
Here the docker-compose with WinstarCloud + Postgresql + Zookeeper + Kafka + **Cassandra**
...
services:
  cassandra:
    ...
    volumes:
      - cassandra:/bitnami
    environment:
      CASSANDRA_CLUSTER_NAME: "WinstarCloud Cluster"
      HEAP_NEWSIZE: "1024M"
      MAX_HEAP_SIZE: "2048M"
  ...
  tb:
    ...
    environment:
      ...
      CASSANDRA_KEYSPACE_NAME: "winstarcloud"
...
In addition to conclusions we made for [Scenario C](#scenario-c), we observe stability of a single WinstarCloud instance handling 100K parallel MQTT connections.
...
* All device connected. Java JMX. VisualVM -> WinstarCloud -> MBeans -> java.lang -> OpenFileDescriptorCount -> more than 100000

{% include images-gallery.html imageCollection="winstarcloud-100k-devices-connected" %}
...
Thanks to the WinstarCloud community that creates issues and shares performance solutions around the time. 
...
It was WinstarCloud 3.3.3 version. We will work on performance improvements in 3.4.x

Feel free to send us feedback or share your fancy setup on [GitHub](https://github.com/winstarcloud/performance-tests/issues)
