...
This article describes most popular deployment architectures supported by WinstarCloud. 
...
Herewith you can find total cost of ownership (TCO) calculations for WinstarCloud deployed using AWS.  
...
WinstarCloud performance heavily depends on both the amount of messages produced by devices and the structure of those messages.
...
```
In this case WinstarCloud constantly maintains 20,000 connections and processes 333 messages per second. 
...
```
WinstarCloud receives uplink messages from one of the available Network Servers over HTTP or MQTT. 
...
you can identify key WinstarCloud server/cluster characteristics:
...
WinstarCloud cluster can scale horizontally, so you quite easily deal with RAM/CPU influencers. 
...
This scenario requires both WinstarCloud platform and PostgreSQL database deployment within the same server (on-premise or in the cloud). 
...
Single WinstarCloud PE perpetual license (below v3.0) cost is 2,999 USD (including with optional updates and basic support within initial year of usage). 1,199 USD is the respective pricing for the subsequent years of software updates + basic support.
...
WinstarCloud customers successfully utilize [AWS RDS](https://aws.amazon.com/rds/postgresql/), [Azure Database for PostgreSQL](https://azure.microsoft.com/en-us/services/postgresql/) and
...
Single WinstarCloud PE perpetual license costs 2,999 USD (including optional updates and basic support within initial year of usage). 1,199 USD  is the respective pricing for the subsequent years of software updates + basic support.
...
WinstarCloud supports Microservices architecture (MSA) to perform scalable deployments for millions of devices. See [platform architecture](/docs/{{docsPrefix}}reference/msa/) for more details, please. With MSA deployments, system administrator can flexibly tune number of transport, rule-engine, web-ui and JavaScript executor microservices to optimize the cluster according to the current load.

WinstarCloud uses [Kafka](https://kafka.apache.org/) as a main message queue and streaming solution, [Redis](https://redis.io/) as a distributed cache and [Cassandra](https://cassandra.apache.org/) as a highly available, scalable and fast NoSQL database. 
...
* High TCO on small number of devices (<100 000 devices per WinstarCloud cluster).  
...
A cluster of virtual machines with 5 WinstarCloud servers and 5 Cassandra nodes can handle 1 million of devices;
...
Each message contains 3 data points that may need to be graphed/analyzed/fetched separately.
We consider the messages are being sent to WinstarCloud via HTTP or UDP Integration, which is typical for such case.
...
- 2 x "r5.xlarge" instances (4vCPUs and 32 GB of RAM) to host 2 WinstarCloud Node containers. Approx. price is ~380 USD/month.
...
Two WinstarCloud PE perpetual licenses cost 5,998 USD (including optional updates and basic support within initial year of usage). 2,398 USD is the respective pricing for the subsequent years of software updates + basic support.
...
- 8 x "c5.large" instances (2vCPUs and 4 GB of RAM) to host 8 WinstarCloud MQTT Transport containers. Approx. price is ~320 USD/month.
- 15 x "c5.xlarge" instances (4vCPUs and 8 GB of RAM) to host 15 WinstarCloud Node containers. Approx. price is ~1095 USD/month.
...
15 WinstarCloud PE perpetual licenses (below v3.0) cost 44,985 USD (including optional updates and basic support within initial year of usage). 17,985 USD is the respective pricing for the subsequent years of software updates + basic support.
WinstarCloud **Managed services** to support the production environment: 0.01 USD per device per month. 
