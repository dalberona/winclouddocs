...
WinstarCloud allows you to generate reports using existing dashboards.
...
On each generate report request, WinstarCloud node sends a request to the Reports Server using the configured endpoint URL.
...
then it captures the dashboard web page into the specified format (*PDF \| PNG \| JPEG*) and sends the captured data as a response to WinstarCloud.

{% if docsPrefix == 'pe/' %}
The system administrator can configure the Reports Server endpoint URL using [winstarcloud.yml](/docs/user-guide/install/pe/config/).
...
Scheduled reports generation is supported by the default **Root Rule Chain** of WinstarCloud PE.
...
WinstarCloud provides access to the generated report files via the **Reports** Widget that is a part of the **Files** Widgets Bundle.
