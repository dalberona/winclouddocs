...
OPC UA Integration allows you to stream data from the OPC UA server to WinstarCloud and converts the device payloads to the WinstarCloud format.
...
In this tutorial, we will configure the integration between WinstarCloud and OPC-UA
...
For sending Downlink messages from the Winstarcloud to the OPC UA node, we need to define a
downlink Converter.
...
The [OPC-UA server](#prerequisites) simulates sending telemetry from devices to WinstarCloud. If you have done everything correctly, 10 new devices should appear on the "**Devices**" page as a result of the integration. Please make sure of this.
