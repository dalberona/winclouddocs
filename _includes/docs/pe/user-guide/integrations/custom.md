...
Custom integration is **only executed remotely** from the main WinstarCloud instance. It allows to create integration with custom configuration 
that will use any transport protocol for communication with your devices.

This guide contains step-by-step instructions on how to create and launch WinstarCloud custom integration.
...
[winstarcloud.cloud](https://winstarcloud.cloud/signup).

Before we start, you can find the full code of custom integration example that we will use in this guide [here](https://github.com/winstarcloud/remote-integration-example).
 
### Prerequisites

We assume you already have a tenant administrator account on your own WinstarCloud PE v2.4.1+ instance or winstarcloud.cloud.
...
Feel free to grab the [code from the WinstarCloud repository](https://github.com/winstarcloud/remote-integration-example) and build the project with maven:
...
```xml
<!-- Api WinstarCloud provides to create custom integration -->
<dependency>
    <groupId>org.winstarcloud.common.integration</groupId>
    <artifactId>remote-integration-api</artifactId>
    <version>${winstarcloud.version}</version>
</dependency>
...
<!-- Grpc transport between remote integration and WinstarCloud -->
<dependency>
...
Main source code is the [CustomIntegration](https://github.com/winstarcloud/remote-integration-example/blob/master/src/main/java/org/winstarcloud/integration/custom/basic/CustomIntegration.java) Java class.
Integration is expecting "Hello to WinstarCloud" message from the TCP client and replies with the "Hello from WinstarCloud!".
Once the [client emulator](https://github.com/winstarcloud/remote-integration-example/blob/master/src/main/java/org/winstarcloud/integration/custom/client/CustomClient.java) receives "Hello from WinstarCloud!"
, it will start sending auto-generated data to WinstarCloud in the following format: **“25,40,94”**. 
The Integration will pass the incoming message as-is to the [uplink converter](/docs/{{peDocsPrefix}}user-guide/integrations/custom/#uplink-converter) and push data to WinstarCloud.

**Note:** starting from WinstarCloud version 3.3.1 a new required configuration property was added to tb-remote-integration.yml:
