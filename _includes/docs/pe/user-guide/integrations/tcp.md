...
TCP Integration allows to stream data from devices which use a TCP transport protocol to WinstarCloud and converts payloads of these devices into the WinstarCloud format.
...
{% endif %}
 - TCP Integration, running externally and connected to the cloud WinstarCloud PE instance;
...
Before setting up an **TCP integration**, you need to create an **Uplink Converter** that is a script for parsing and transforming the data received by TCP integration to a format that WinstarCloud can consume.
...
Once WinstarCloud TCP Integration has been created, the TCP server starts, and then it waits for data from the devices.
...
An example of a sent message and a response from WinstarCloud in the terminal:

![image](/images/user-guide/integrations/tcp/tcp-terminal-send-downlink-message.png)

This command will send the Uplink message to the WinstarCloud and will wait for Downlink message for 60 seconds if the message exists. 
