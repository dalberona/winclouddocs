...
MQTT Integration allows to connect to external MQTT brokers, subscribe to data streams from those brokers and convert any type of payload from your devices to WinstarCloud message format. Its typical use is whenever your devices are already connected to external MQTT broker or any other IoT platform or connectivity provider with MQTT based back-end. 

Please review the integration diagram to learn more. 

<object width="100%" style="max-width: max-content;" data="/images/user-guide/integrations/mqtt-integration.svg"></object>

WinstarCloud MQTT Integration acts as an MQTT client. It subscribes to topics and converts the data into telemetry and attribute updates. In case of downlink message, MQTT integration converts it to the device-suitable format and pushes to external MQTT broker. 
Pay attention: MQTT broker should be either co-located with WinstarCloud instance or deployed in the cloud and have a valid DNS name or static IP address. WinstarCloud instance that is running in the cloud can’t connect to the MQTT broker deployed in local area network.
...
- WinstarCloud Professional Edition instance — [winstarcloud.cloud](https://winstarcloud.cloud);
  {% endif %}
 - MQTT broker, accessible by WinstarCloud PE instance — broker.hivemq.com (port 1883);
...
Downlink converter parses and transforms the data sent from WinstarCloud to the format that is consumed by existing device(s).
