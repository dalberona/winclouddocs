...
Sigfox Integration allows to stream data from Sigfox Backend to WinstarCloud and converts binary device payloads to the WinstarCloud format.
...
You can сreate an uplink converter in the Data converters page or directly in the integration. Uplink converter is necessary in order to convert the incoming data from the device into the required format for displaying them in WinstarCloud.
...
**NOTE**
<br>
If the "Allow create devices or assets" checkbox is unchecked, when sending a message to winstarcloud with any parameters of the device (or asset), if such a device (asset) does not exist, then device (asset) will not be created.
{% endcapture %}
{% include templates/info-banner.md content=difference %}
...
Now we need to set up a **Sigfox account** so that data from our device is sent to the WinstarCloud platform.
...
Create a callback to connect the Sigfox cloud to your WinstarCloud platform. In the upper right corner, click on the "**New**" button, and select "**Custom callback**".
...
After this, the device is ready to send data to Winstarcloud. Send an uplink message from the device. 

After you sent the uplink message a new device was created in Winstarcloud. You should receive a **notification** about it. To view notification click on the "bell" icon in the upper right corner of the screen.
