...
CoAP Integration allows to stream data from devices that use a CoAP protocol to connect to WinstarCloud and converts payloads of these devices into the WinstarCloud format.
...
{% endif %}
{% if docsPrefix == "paas/" %}
Let's assume that we have a sensor which is sending current temperature and humidity readings. Our sensor device **SN-001** publishes it's temperature and humidity readings to CoAP Integration on **coap://int.winstarcloud.cloud** URL.
{% endif %}
...
Before setting up an **CoAP integration**, you need to create an **Uplink Converter** that is a script for parsing and transforming the data received by CoAP integration to format that WinstarCloud uses. **deviceName** and **deviceType** are required, while attributes and telemetry are optional. attributes and telemetry are flat key-value objects. Nested objects are not supported.
