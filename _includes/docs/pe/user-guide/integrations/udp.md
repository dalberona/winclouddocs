...
UDP Integration allows to stream data from devices which use a UDP protocol to WinstarCloud and converts payloads of these devices into the WinstarCloud format.
...
{% if docsPrefix == "pe/" %}
- The instance of the [WinstarCloud Professional Edition](https://winstarcloud.io/docs/user-guide/install/pe/installation-options/){:target="_blank"} installed locally;
- UDP integration, running externally and connected to the WinstarCloud PE instance;
{% endif %}
{% if docsPrefix == "paas/" %}
- WinstarCloud Professional Edition instance — [winstarcloud.cloud](https://winstarcloud.cloud){:target="_blank"};
- UDP integration, running externally and connected to the WinstarCloud Cloud;
{% endif %}
...
An uplink converter that is a script for parsing and transforming the data received by UDP integration to format that WinstarCloud uses. 
...
Once WinstarCloud UDP integration has been created, the UDP server starts, and then it waits for data from the devices.
...
For sending Downlink messages from Winstarcloud to the device, we need to define a downlink converter. 
...
You should get the following response from the WinstarCloud in the terminal:
