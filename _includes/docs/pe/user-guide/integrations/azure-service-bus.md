...
Azure Service Bus Integration allows to stream data from Azure Service bus to WinstarCloud and converts device payloads to the WinstarCloud format.

![image](/images/user-guide/integrations/azure-service-bus/azure-service-bus-overview.png)

## Configure Azure Service Bus

To create WinstarCloud Service Bus integration you will need two things created in Azure portal: **topic** and **subscription** to the topic. 
...
data from the device into the required format for displaying them in WinstarCloud. Click on the “plus” and on “Create new converter”. To view the events, enable Debug. 
...
After test uplink message was sent integration has created the device inside Winstarcloud. Let`s check with which Rule Chain it is connected.
For this go to **Entities** -> **Devices** page in Winstarcloud menu and find the device with the name that we have used in the uplink. Find and navigate to the **device profile** used by the device.
