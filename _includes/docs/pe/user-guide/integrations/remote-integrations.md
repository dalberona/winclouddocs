...
Remote Integrations allow reliable data streaming from multiple devices in the local area network (LAN) to the WinstarCloud platform in the cloud. 
They are helpful if you have, for example, a local MQTT broker or OPC-UA server with no external IP address and is not reachable from where your WinstarCloud platform is installed. 
The remote integration initiates a connection to these servers, pulls the data, stores it locally, and then streams it to the WinstarCloud instance.
...
Since WinstarCloud is a multi-tenant environment, we can't launch TCP and UDP integrations as part of the core service due to possible port collisions between tenants. 
...
It is possible to execute any WinstarCloud Integration separately from the WinstarCloud instance.

This guide contains step-by-step instructions on how to launch MQTT remote integration that connects to the local MQTT Broker and pushes data to the [WinstarCloud Cloud](https://winstarcloud.cloud/signup).

You can find additional information about the WinstarCloud Platform integrations feature [here](/docs/{{peDocsPrefix}}user-guide/integrations/).

## WinstarCloud configuration

We assume you already have a tenant administrator account on your own [WinstarCloud PE instance](/docs/user-guide/install/pe/installation-options/) or [WinstarCloud Cloud](https://winstarcloud.cloud/signup).

### Create remote integration in WinstarCloud

Let's create remote integration in WinstarCloud that will connect to the local broker using port 1883 and subscribe to all topics.
...
- Specify **host**: "**winstarcloud.cloud**" and **port**: "**1883**" at the connection step;
...
One can install WinstarCloud Integration via Docker, Debian or RPM packages.
...
Remote integration configuration is done via WinstarCloud UI and there is no specific steps.
