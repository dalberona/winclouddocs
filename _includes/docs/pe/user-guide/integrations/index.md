...
WinstarCloud Platform integrations feature was designed for two primary use cases / deployment options:

  - Connect existing NB IoT, LoRaWAN, SigFox and other devices with specific payload formats directly to WinstarCloud platform.
...
The job of WinstarCloud Integration is to provide secure and reliable API bridge between core platform features (telemetry collection, attributes and RPC calls) and specific third-party platform APIs.    

### How it works?

At the moment WinstarCloud supports various integration protocols. Most popular are HTTP, MQTT and OPC-UA. 
...
Once message arrives from External Platform to WinstarCloud it passes validation according to platform specific payload format and security rules. 
Once message is validated WinstarCloud Integration invokes assigned [**Uplink Data Converter**](/docs/{{peDocsPrefix}}user-guide/integrations/#uplink-data-converter) to extract sub-set of meaningful information out of the incoming message.
The message is basically transformed from device and platform specific payload to the format that WinstarCloud uses.
...
Once message is pushed by the rule engine, WinstarCloud invokes assigned [**Downlink Data Converter**](/docs/{{peDocsPrefix}}user-guide/integrations/#downlink-data-converter) and transforms the rule engine message to the specific data format that is used by the Integration.
...
Embedded integration is running in the main WinstarCloud server process. Basically it is part of a monolith deployment scenario.
...
Cons:
  * consume resources allocated to main WinstarCloud process: network connections, OS threads and CPU cycles;
  * low level of isolation;
  * can't access local MQTT brokers or OPC-UA servers if WinstarCloud is deployed in the cloud.
  
<object width="60%" data="/images/user-guide/integrations/embeded-integrations-overview.svg"></object> 
  
#### Remote integrations
 
Remote integration become available since WinstarCloud PE v2.4.1 and enables new deployment scenario. 
...
Those brokers and/or servers don't have dedicated external IP address, so WinstarCloud instance in the cloud can't connect to them directly. 
...
Remote integration will stream the data to the WinstarCloud instance deployed in the cloud once the internet connection is available.

Pros:
  * enables integration with servers deployed in the local network;
  * isolates the integration process from main WinstarCloud process;
...
The main function of **Uplink Data Converter** is to parse payload of the incoming message and transform it to format that WinstarCloud uses.
...
Starting version 2.4.2, WinstarCloud also supports **assetName** and **assetType** instead of deviceName and deviceType.
{% endcapture %}
...
**NOTE**:
<br>
Starting version 2.4.2, WinstarCloud also support optional **customerName** and **groupName**.
Those parameters will cause WinstarCloud to automatically create customer and/or entity group and assign those entities to the customer and/or group.
{% endcapture %}
{% include templates/info-banner.md content=difference %}
...
Let's assume an example where temperature and humidity upload frequency attributes are updated via WinstarCloud REST API and 
...
Experienced WinstarCloud users may notice that functionality of Integrations feature partially overlap with functionality of [IoT Gateway](/docs/iot-gateway/what-is-iot-gateway/).
However, there are key differences between these two systems/features:
  ...
  - IoT Gateway is designed to support < 1000 devices, while Integrations are designed for high throughput, scalability and cluster deployments as part of WinstarCloud server.
...
Please note that you can share your converters with community and send them to us to make part of official WinstarCloud distributive.   
