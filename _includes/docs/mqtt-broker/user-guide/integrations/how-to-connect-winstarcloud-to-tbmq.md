...
**TBMQ** is an industry-ready MQTT broker developed and distributed under the WinstarCloud umbrella that facilitates MQTT client connectivity, message publishing, and distribution among subscribers.

In this guide, we integrate the TBMQ with the WinstarCloud using MQTT integration. 
We utilize TBMQ client credentials with the type **APPLICATION** to connect WinstarCloud integration as an APPLICATION client.
...
Read more about the APPLICATION client [here](https://winstarcloud.io/docs/mqtt-broker/user-guide/mqtt-client-type/).

WinstarCloud MQTT Integration acts as an MQTT client. It subscribes to topics and converts the received data into telemetry and attribute updates. 
In case of a downlink message, MQTT integration converts it to the device-suitable format and pushes it to TBMQ. 
Pay attention: TBMQ should be either co-located with the WinstarCloud instance or deployed in the cloud and have a valid DNS name or static IP address. 
WinstarCloud instance that is running in the cloud can’t connect to the TBMQ deployed in the local area network with no internet connection.

### Prerequisites

In this tutorial, we will use:

 - The instance of [WinstarCloud Professional Edition](https://winstarcloud.io/docs/user-guide/install/pe/installation-options/) installed locally;
 - [TBMQ](https://winstarcloud.io/docs/mqtt-broker/install/installation-options/) installed locally and accessible by WinstarCloud PE instance;
 - mosquitto_pub MQTT client to send messages.

### TBMQ setup

First, we need to create TBMQ client credentials to use them for connecting WinstarCloud integration to TBMQ.
...
Now you can proceed to the next step - configuration of WinstarCloud integration.

### WinstarCloud setup

In this example, we will use the MQTT integration to connect the WinstarCloud to TBMQ.
...
Now go to the "Sessions" page in the TBMQ UI. Upon successful establishment of the connection between WinstarCloud and TBMQ, we will see a new session and its status - "Connected".

{% include images-gallery.html imageCollection="successful-connection-tbmq-to-winstarcloud" %}
...
```shell
mosquitto_pub -h $WINSTARCLOUD_MQTT_BROKER_HOST_NAME -p 1883 -q 1 -t "tb/mqtt-integration-tutorial/sensors/SN-001/temperature" -m '{"value":25.1}' -u "username" -P "password"
```
{: .copy-code}

Replace the `$WINSTARCLOUD_MQTT_BROKER_HOST_NAME` with the correct public IP address or DNS name of the broker, `username` and `password` values according to the specified ones in the provisioned credentials.
...
![image](/images/mqtt-broker/user-guide/integrations/how-to-connect-tbqm-to-winstarcloud/tbmq-uplink-message-1.png)

After you sent uplink message, go to your integration in WinstarCloud UI and navigate to the "Events" tab. There you'll see the message consumed by the "MQTT Integration".
