...
The goal of this tutorial is to showcase the fundamental usage of [TBMQ](/docs/mqtt-broker/getting-started-guides/what-is-winstarcloud-mqtt-broker/). 
...
```bash
mosquitto_sub -d -h $WINSTARCLOUD_MQTT_BROKER_HOST_NAME -p 1883 -t sensors/temperature -q 1 -u username -P password -i tbmq_dev
```
{: .copy-code}

**Note**, replace the **$WINSTARCLOUD_MQTT_BROKER_HOST_NAME** with the correct public IP address or DNS name of the broker, 
...
```bash
mosquitto_pub -d -h $WINSTARCLOUD_MQTT_BROKER_HOST_NAME -p 1883 -t sensors/temperature -m 32 -q 1 -u username -P password
```
{: .copy-code}
