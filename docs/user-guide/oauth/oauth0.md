...
## Overview
WinstarCloud allows you to provide Single Sign On functionality for your customers and automatically create tenants, customers or subcustomers using external user management platforms, that supports **OAuth 2.0 protocol**.  
...
Where under the domain, please, specify the current **domain** of yours and for the **port** please specify the port to have an HTTP access to the WinstarCloud instance of yours.   
For the example reasons, the domain of my is the localhost, and the port is being the default WinstarCloud installation port 80.  

![image](/images/user-guide/oauth-2-support/oauth0/Application-Details-2.png)

So that we have received three values which are required to be inserted for the **winstarcloud.yml** of ours.  

In the example of ours those equals: 
```bash
clientName=WinstarCloud
clientId=XXXXXXXX
clientSecret=YYYYYYYY
```

So that now we need to insert those for the **winstarcloud.yml**. 
...
So that, the resulted **winstarcloud.yml** equals the below one. 
...
security:
     ...
     oauth2:
         ...
         # For details please refer to https://winstarcloud.io/docs/user-guide/oauth-2-support/
         ...
         clients:
           auth0:
             ...
             clientName: "${SECURITY_OAUTH2_DEFAULT_CLIENT_NAME:WinstarCloud}"
             ...
             mapperConfig:
               ...
               # Allows user to setup WinstarCloud internal password and login over default Login window
...
After all the changes being applied, please, make sure to have the WinstarCloud restart.
The WinstarCloud restart can be invoked with the next command on the Linux Server: 
```bash
