---
layout: docwithnav
title: Connect WinstarCloud to Datastax Astra DB 
description: Connect WinstarCloud to Datastax Astra DB
hidetoc: "true"
---

This guide describes how to setup Datastax Astra DB cloud for your WinstarCloud. This is an alternative having your own Cassandra cluster.
...
Create a new database `winstarcloud` with keyspace `winstarcloud`. Select the closest location to you and cloud provider you like. 
...
You need a `Database administrator` role to install the Winstarcloud schema. Click `Create a token` on the `winstarcloud` database.
...
The `secure-connect-winstarcloud.zip` file contains the security certificates and credentials for your database

### Step 5. Set connection parameters on WinstarCloud

Here the environments you need to set to use your Astra DB with WinstarCloud.

#### WinstarCloud service

For WinstarCloud as a local service add to the `/etc/winstarcloud/conf/winstarcloud.conf`

```bash
export DATABASE_TS_TYPE=cassandra
# Cassandra on Astra DB cloud 
export CASSANDRA_CLOUD_SECURE_BUNDLE_PATH=/etc/winstarcloud/astra/secure-connect-winstarcloud.zip
# dbadmin
...
services:
  tb:
    image: "winstarcloud/tb-postgres:3.4.2"
    network_mode: "host"
    restart: "always"
    # replace /home/your_user/Downloads/ with your local path
    volumes:
      - /home/your_user/Downloads/secure-connect-winstarcloud.zip:/etc/winstarcloud/astra/secure-connect-winstarcloud.zip
    environment:
      HTTP_BIND_PORT: "8080"
      DATABASE_TS_TYPE: "cassandra"
      # Cassandra on cloud
      CASSANDRA_CLOUD_SECURE_BUNDLE_PATH: "/etc/winstarcloud/astra/secure-connect-winstarcloud.zip"
      # dbadmin
...
#### WinstarCloud Kubernetes cluster
...
```bash
base64 -w 0 /home/your_user/projects/astra/secure-connect-winstarcloud.zip | xclip -selection clipboard
```
{: .copy-code}

The resulting file will look like `astra-secret.yml`. The `secure-connect-winstarcloud.zip` encoded string is very long.
```yaml
apiVersion: v1
kind: Secret
metadata:
...
type: Opaque
data:
  secure-connect-winstarcloud.zip: >-
...
Then mount `secure-connect-winstarcloud.zip` from secret to the `tb-node.yml` and `tb-rule-engine.yml`.
      ...
      volumes:
        - name: astra-secret
          secret:
            ...
            items:
              - key: secure-connect-winstarcloud.zip
                path: secure-connect-winstarcloud.zip
      containers:
        - name: tb-node
          image: winstarcloud/tb-node:3.4.2
          volumeMounts:
            - mountPath: /etc/winstarcloud/astra
              name: astra-secret
              readOnly: true
          env:
            ...
            - name: CASSANDRA_CLOUD_SECURE_BUNDLE_PATH
              value: "/etc/winstarcloud/astra/secure-connect-winstarcloud.zip"
...
### Step 6. Run standard WinstarCloud install

WinstarCloud is already support install on Astra DB cloud.
...
### Winstarcloud test run on Astra DB cloud

Here an example how to test your Winstarcloud with your Astra DB using performance test tool.
To spin up the Winstarcloud locally, use `docker-compose.yml` provided couple sections above.
...
docker run -it --rm --network host --name tb-perf-test \
           ...
           --env REST_USERNAME=tenant@winstarcloud.org \
           ...
           winstarcloud/tb-ce-performance-test:latest
```
{: .copy-code}
