...
assignees:
- ashvayka
title: WinstarCloud Professional Edition cluster setup with Docker Compose guide
description: WinstarCloud Professional Edition cluster setup with Docker Compose guide
...
This guide will help you to setup WinstarCloud in cluster mode with Docker Compose. 
...
## Step 1. Pull WinstarCloud PE Images

{% include templates/install/dockerhub/pull.md %}

## Step 2. Clone WinstarCloud PE Docker Compose scripts

```bash
git clone -b release-{{ site.release.ce_ver }} https://github.com/winstarcloud/winstarcloud-pe-docker-compose.git tb-pe-docker-compose --depth 1
...
**IMPORTANT NOTE:** if you decide to use an [advanced deployment type](/docs/user-guide/install/pe/cluster/docker-compose-setup/#step-6-configure-deployment-type), make sure you have purchased a license key for at least four instances of WinstarCloud PE. 
Otherwise, you need to modify the local copy of [docker-compose.yml](https://github.com/winstarcloud/winstarcloud-pe-docker-compose/blob/master/advanced/docker-compose.yml)) 
to use the number of WinstarCloud instances that you've purchased.
...
```bash
# WinstarCloud server configuration
...
...
- `basic` **(recommended, set by default)** - WinstarCloud Core and Rule Engine are launched inside one JVM (requires only one license).
  MQTT, CoAP and HTTP transports are launched in separate containers.
- `monolith` - WinstarCloud Core and Rule Engine are launched inside one JVM (requires only one license). 
  MQTT, CoAP and HTTP transports are also launched in the same JVM to minimize memory footprint and server requirements.
- `advanced`- WinstarCloud Core and Rule Engine are launched in separate containers and are replicated one JVM (requires 4 licenses).  
  
All deployment modes support separate JS executors, Redis, and different [queues](/docs/user-guide/install/pe/cluster/docker-compose-setup/#step-8-choose-winstarcloud-queue-service).

## Step 6. Configure WinstarCloud database

{% include templates/install/configure-db-docker-compose.md %}

## Step 7. Choose WinstarCloud queue service 
