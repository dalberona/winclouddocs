...
assignees:
- ashvayka
title: Installing WinstarCloud PE using Docker (Linux or Mac OS)
description: Installing WinstarCloud PE IoT Platform using Docker (Linux or Mac OS)
...
This guide will help you to install and start WinstarCloud Professional Edition (PE) using Docker and Docker Compose on Linux or Mac OS. 
...
- `~/.mytbpe-data:/data`   - mounts the host's dir `~/.mytbpe-data` to WinstarCloud data directory;
- `~/.mytbpe-data/db:/var/lib/postgresql/data`   - mounts the host's dir `~/.mytbpe-data/db` to Postgres data directory;
- `~/.mytbpe-logs:/var/log/winstarcloud`   - mounts the host's dir `~/.mytbpe-logs` to WinstarCloud logs directory;
...
- `winstarcloud/tb-pe:{{ site.release.pe_full_ver }}`          - docker image.
...
```bash
docker compose exec postgres sh -c "pg_dump -U postgres winstarcloud > /var/lib/postgresql/data/winstarcloud_dump"
```
{: .copy-code}

{% capture dockerComposeStandalone %}
If you still rely on Docker Compose as docker-compose (with a hyphen) execute next command:
<br>**docker-compose exec postgres sh -c "pg_dump -U postgres winstarcloud > /var/lib/postgresql/data/winstarcloud_dump"**
{% endcapture %}
{% include templates/info-banner.md content=dockerComposeStandalone %}

* After this you need to update docker-compose.yml as in [Step 3](#step-3-choose-winstarcloud-queue-service) but with 3.1.1PE instead of 3.2.2PE:

* Change upgradeversion variable to your **current** WinstarCloud version.
...
To upgrade WinstarCloud to the latest version those steps should be done **for each intermediate version**.
...
```bash
docker compose exec postgres sh -c "psql -U postgres -c 'drop database winstarcloud'"
docker compose exec postgres sh -c "psql -U postgres -c 'create database winstarcloud'"
docker compose exec postgres sh -c "psql -U postgres winstarcloud < /var/lib/postgresql/data/winstarcloud_dump"
```
{: .copy-code}

{% capture dockerComposeStandalone %}
If you still rely on Docker Compose as docker-compose (with a hyphen) here is the list of the above commands:
<br>**docker-compose exec postgres sh -c "psql -U postgres -c 'drop database winstarcloud'"**
<br>**docker-compose exec postgres sh -c "psql -U postgres -c 'create database winstarcloud'"**
<br>**docker-compose exec postgres sh -c "psql -U postgres winstarcloud < /var/lib/postgresql/data/winstarcloud_dump"**
{% endcapture %}
{% include templates/info-banner.md content=dockerComposeStandalone %}
...
```
docker cp tb-docker_postgres_1:/var/lib/postgresql/data/winstarcloud_dump .
```
{: .copy-code}
