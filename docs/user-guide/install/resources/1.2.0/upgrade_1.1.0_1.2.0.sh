...
function selectSystemWidgetType {
   bundleAlias=$1
   alias=$2
   cqlsh -e "select id from winstarcloud.widget_type_by_tenant_and_aliases where tenant_id = 13814000-1dd2-11b2-8080-808080808080 and bundle_alias = '$bundleAlias' and alias = '$alias'" | sed -n 4p | awk '{ print $1 }'
}

function selectFirstSystemWidgetType {
   cqlsh -e "select id from winstarcloud.widget_type_by_tenant_and_aliases where tenant_id = 13814000-1dd2-11b2-8080-808080808080" | sed -n 4p | awk '{ print $1 }'
}
...
function deleteSystemWidgetType {
   ...
   widgetTypeId=$( selectSystemWidgetType $bundleAlias $alias )
	...
	echo Will delete widgetTypeId [$widgetTypeId]
        cqlsh -e "delete from winstarcloud.widget_type where id=$widgetTypeId"
    else 
