...
for /F "usebackq skip=3 tokens=1 eol=(" %%a in (`%CQLSH% -e "select id from winstarcloud.widget_type_by_tenant_and_aliases where tenant_id = minTimeuuid ( 0 ) and bundle_alias='%1' and alias='%2'"`) do (	
	set widgetTypeId=%%a
	@echo Deleting widget type with id = !widgetTypeId!
	call %CQLSH% -e "delete from winstarcloud.widget_type where id = !!widgetTypeId!!">nul
)
