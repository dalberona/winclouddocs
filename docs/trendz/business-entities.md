...
This guide describes how Trendz uses entities from WinstarCloud, like asset, device, relation, etc.
...
Now let's check how Trendz resolves data from WinstarCloud using following report: we are using only 2 fields from Smart Building topology: 
...
* Trendz will find all available buildings in the WinstarCloud. 
