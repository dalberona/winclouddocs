...
heatpumps-create-anomaly-alert:
  0:
    ...
    title: 'Create anomaly score alert for heat pumps in WinstarCloud device profile'
  1:
...
heatpumps-notify-about-anomalies:
  0:
    ...
    title: 'Open root rule chain in WinstarCloud'
  1:
...
### Prerequisites
Heat pump reports telemetry data to WinstarCloud via MQTT protocol. They registered in platform as a device entity. Devices have relation to Apartment asset and apartments assigned to customer account. 
...
#### Save current anomaly score for heat pump as a telemetry back to WinstarCloud
...
At this moment we already have `heatpumpAnomalyScore` telemetry for each heat pump in the WinstarCloud which tells as how abnormal its current behavior. It means that we can create Alarm Rule in ThignsBoard to raise an alarm if score index is higher than 50.

* In WinstarCloud open heat pump's device profile and add new Alarm Rule
...
With this configuration WinstarCloud will send notification to the maintenance team once anomaly was detected in heat pumps behavior.
