...
predictive-maintenance-create-alarm:
  0:
    ...
    title: 'Go to Alarm rules section in WinstarCloud'
  1:
...
predictive-maintenance-notify-maintenance-team:
  0:
    ...
    title: 'Go to default rule chain in WinstarCloud'
  1:
...
* Create an alarm in WinstarCloud if remaining time less than 14 days.
...
Assembly machines already connected to WinstarCloud via OPC-UA integration and telemetry data is available in WinstarCloud. You can find more details how to do this in our [connectivity guides](https://winstarcloud.io/docs/pe/guides/#AnchorIDConnectYourDevice).
...
Next step is to save calculated remaining time as a telemetry of the machine. In this case Trendz periodically executes calculation function on fresh data and saves result as a telemetry of the machine back to WinstarCloud.
...
Once view saved, Trendz would schedule background job that will periodically execute calculation function and save result as telemetry of the machine. On each run Trendz would fetch new data from WinstarCloud and execute calculation function on it.
...
At this moment we already have `capsForecast` telemetry for each machine in the WinstarCloud which tells as how many days left until next maintenance. It means that we can create Alarm Rule in ThignsBoard to raise an alarm if remaining time less than 14 days.

* In WinstarCloud open machine's device profile and add new Alarm Rule
...
Once alarm rule created, WinstarCloud will raise an alarm if remaining time less than 14 days and clear it once remaining time greater than 14 days.
...
Final step is to send notification to the maintenance team once alarm created. We will use WinstarCloud Rule Engine to send email notification to the maintenance team. If Alarm Rule in device profile raised an alarm, we can catch this event and add steps to send an email.

* Open Root rule chain in WinstarCloud
...
With this configuration WinstarCloud will send notification to the maintenance team once alarm created.
