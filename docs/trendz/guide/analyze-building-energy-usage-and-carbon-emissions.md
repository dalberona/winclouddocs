...
building_energy_dashboard:
  0:
    ...
    title: 'Create new dashboard in WinstarCloud'
  1:
...
* Create WinstarCloud dashboard to visualize data with filtering options

## Getting started:

### Prerequisites
Energy meters already installed in the building and connected to WinstarCloud via MQTT API. To simplify data aggregation and analysis, each meter has 2 attributes:
...
### Step 6: Create energy consumption analytic dashboard in WinstarCloud
...
consumption sources and areas. We begin with adding all Trendz views that we created on one dashboard in WinstarCloud:

* In WinstarCloud create dashboard with name **Energy consumption report**
...
* Return to WinstarCloud **Energy consumption report** dashboard and tune dashboard layout.
...
* Add WinstarCloud table widget `Cards` -> `Entities table` to the dashboard. It will show all buildings that are visible for the user.
