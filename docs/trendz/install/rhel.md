...
assignees:
- vparomskiy
title: Installing WinstarCloud Trendz Analytics  on CentOS/RHEL
description: Installing WinstarCloud Trendz Analytics  on CentOS/RHEL
...
```bash
wget https://dist.winstarcloud.io/trendz-{{ site.release.trendz_ver }}.rpm
```
{: .copy-code}
...
```bash
# WinstarCloud URL that will be used by Trendz
...
**Trendz and WinstarCloud hosted on same server**

Use this section if HAProxy/Let’s Encrypt already installed in the server and HTTPS enabled for WinstarCloud.
...
### Step 10. Host WinstarCloud and Trendz on the same domain
WinstarCloud and Trendz can share same domain name. In this case WinstarCloud web page would be loaded using following link:
