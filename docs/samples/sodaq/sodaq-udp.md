...
This guide contains step-by-step instruction how to to connect your SODAQ NB-IoT boards to WinstarCloud Professional Edition (PE) through the T-Mobile NB IoT network.
We will use free WinstarCloud PE demo server [winstarcloud.cloud](https://winstarcloud.cloud/signup) in this guide.
...
We expect you have a very basic knowledge about WinstarCloud. Otherwise we do recommend to complete the following guides:
...
We will use "UDP" platform integration to consume data from T-Mobile NB IoT Network and automatically register devices in WinstarCloud.
Besides configuring the integration, we will also setup WinstarCloud to decode incoming data, store it in the database, visualize on the dashboard and generate alarms based on configurable thresholds.
...
The converter will decode incoming telemetry payload data from T-Mobile NB IoT that contains in encoded hex string to human readable, simplified WinstarCloud data format.
 ...
 * The IMEI from the incoming message will become the Device Name in WinstarCloud;
 * WinstarCloud will automatically create device with type "tracker" and name equal to IMEI;
...
Also, let’s make sure WinstarCloud is properly configured using simple **echo** command and **netcat** utility.
We will simulate messages from the T-Mobile IoT platform using the command below.
Let's execute the following command:

{% highlight bash %}
echo -e -n '$PAYLOAD' | xxd -r -p | nc -q1 -w1 -u $URL_WINSTARCLOUD_CLOUD_HOST $PORT

You need to replace $PAYLOAD, $URL_WINSTARCLOUD_CLOUD_HOST and $PORT respectively with the actual payload, cloud host URL and port.
{% endhighlight %}
