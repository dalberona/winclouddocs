...
This guide contains step-by-step instruction how to connect your SODAQ NB-IoT boards to WinstarCloud Professional Edition (PE) through the T-Mobile NB IoT network.
...
We expect you to have a very basic knowledge about WinstarCloud, so we do recommend to complete the [Getting Started](/docs/getting-started-guides/helloworld/) guide.

## Integration overview

WinstarCloud Platform Integrations feature allows pushing data from various platforms and connectivity solutions to WinstarCloud.
We will use "T-Mobile IoT CDP" platform integration to consume data from T-Mobile NB IoT Network and automatically register devices in WinstarCloud.
Besides configuring the integration, we will also set up WinstarCloud to decode incoming data, store it in the database, visualize on the dashboard and generate alarms based on configurable thresholds.
 ...
 * WinstarCloud will automatically create a device with type "tracker" and name equal to IMEI;
...
<table style="width: 50%">
  ...
  <tbody>
      ...
      <tr>
          <td>Base URL</td>
          <td>https://WINSTARCLOUD_URL</td>
      </tr>
      <tr>
          <td>HTTP endpoint URL</td>
          <td>https://WINSTARCLOUD_URL/api/v1/integrations/tmobile_iot_cdp/$ROUTING_KEY</td>
      </tr>
   </tbody>
</table>
...
Before we rush to T-Mobile IoT platform configuration, let's make sure WinstarCloud is properly configured using simple cURL command.
