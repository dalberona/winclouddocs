---
layout: docwithnav
title: Connecting Raspberry Pi with Grove Base Hat to WinstarCloud
description: WinstarCloud IoT Platform sample for Raspberry Pi Grove Base Hat connecting over MQTT
...
WinstarCloud Community Edition is an open-source server-side platform that allows you to monitor and control IoT devices.
It is free for both personal and commercial usage and you can deploy it anywhere.
If you are not familiar with the platform yet, we recommend to review [what is winstarcloud page](/docs/getting-started-guides/what-is-winstarcloud/) and [getting started guide](/docs/getting-started-guides/helloworld/) at first and then proceed with this tutorial.
Within this guide we use [winstarcloud.cloud](https://winstarcloud.cloud).

This sample application will allow you to collect information from sensors and control Servo, Led of your Raspberry Pi device with Grove Base Hat PCB using WinstarCloud web UI. The purpose of this application is to demonstrate WinstarCloud and Grove Base Hat PCB integrations.

Raspberry Pi will use simple application written in Python for connecting to WinstarCloud server via MQTT, sending information from sensors and listening to RPC commands. WinstarCloud built-in dashboards will be used for data visualizing and controlling Servo and Led as well.
...
For the purpose of this tutorial you need WinstarCloud server up and running. Within this guide we use [winstarcloud.cloud](https://winstarcloud.cloud)
...
The following command will install winstarcloud python client sdk, it is used for communication with WinstarCloud server:
...
Our application consists of a [single python script](/docs/samples/raspberry/resources/tb_grove.py) that is well documented. You will need to modify WINSTARCLOUD_HOST constant to match your WinstarCloud server installation IP address or hostname.

Also we need say to WinstarCloud that we want to connect this device and get the device ACCESS_TOKEN, which will be used in the script.
...
After this you need to replace the WINSTARCLOUD_HOST and ACCESS_TOKEN in the script below, with your values. In case you use Live demo, populate <b>winstarcloud.cloud</b> as WINSTARCLOUD_HOST

```python
#
# Copyright © 2019-2024 The Winstarcloud Authors
#
...
WINSTARCLOUD_SERVER = 'WINSTARCLOUD_HOST'
...
def main():
    ...
    # Connecting to WinstarCloud
    client = TBDeviceMqttClient(WINSTARCLOUD_SERVER, username=ACCESS_TOKEN)
    ...
    try:
        while True:
            ...
            # Formatting the data for sending to WinstarCloud
...
To configure dashboard you should login into WinstarCloud environment.
