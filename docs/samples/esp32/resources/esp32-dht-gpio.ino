...
#include <WinstarCloud.h>    // WinstarCloud SDK
...
#define TOKEN               "TOKEN"
// WinstarCloud server instance.
#define WINSTARCLOUD_SERVER  "demo.winstarcloud.io"

// Baud rate for debug serial
#define SERIAL_DEBUG_BAUD    115200

// Initialize WinstarCloud client
WiFiClient espClient;
// Initialize WinstarCloud instance
WinstarCloud tb(espClient);
// the Wifi radio's status
int status = WL_IDLE_STATUS;

// Array with LEDs that should be lit up one by one
uint8_t leds_cycling[] = { 25, 26, 32 };
// Array with LEDs that should be controlled from WinstarCloud, one by one
...
void loop() {
  ...
  // Reconnect to WinstarCloud, if needed
  if (!tb.connected()) {
    subscribed = false;
    ...
    if (!tb.connect(WINSTARCLOUD_SERVER, TOKEN)) {
  ...
  if (send_passed > send_delay) {
    ...
    // Uploads new telemetry to WinstarCloud using MQTT. 
    // See https://winstarcloud.io/docs/reference/mqtt-api/#telemetry-upload-api 
    // for more details
