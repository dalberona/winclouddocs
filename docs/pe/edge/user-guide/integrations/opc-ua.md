...
OPC UA Integration allows you to stream data from the OPC UA server to WinstarCloud Edge and converts the device payloads to the WinstarCloud Edge format.

<object width="100%" style="max-width: max-content;" data="/images/user-guide/integrations/opc-ua-integration.svg"></object>

In this tutorial, we will configure the integration between WinstarCloud Edge and OPC-UA 
...
**Downlink Converter** parses and transforms the data sent from WinstarCloud Edge to the format that is consumed by existing device(s).
...
The purpose of the decoder function is to parse the incoming data and metadata to a format that WinstarCloud Edge can consume.
...
For sending Downlink messages from the WinstarCloud Edge to the OPC UA node, we need to define a Downlink Converter.
