...
2. General device configuration. In this section we have defined general device settings, such as device name in WinstarCloud ("Temperature and humidity sensor"), MAC-address ("4C:65:A8:DF:85:C0") etc. You can read more about available parameters [here](/docs/iot-gateway/config/ble/#device-object-subsection).
...
4. Attributes configuration. In this section we have defined that connector will read value from characteristic ("00002A00-0000-1000-8000-00805F9B34FB") and write it as the device client-side attribute ("name") on WinstarCloud. You can read more about available parameters [here](/docs/iot-gateway/config/ble/#subsection-attributes).    
5. Attribute updates configuration. In this section we have configured the gateway to change the device name when we change shared attribute ("sharedName") in WinstarCloud device. You can read more about available parameters [here](/docs/iot-gateway/config/ble/#subsection-attributeupdates).   
6. Server side rpc configuration. In this section we have configured the gateway to read the device name and return it when we call RPC method ("rpcMethod1") from WinstarCloud. You can read more about available parameters [here](/docs/iot-gateway/config/ble/#subsection-serversiderpc).   
...
## Step 5. Check information on WinstarCloud

Check data in your WinstarCloud instance, that you have been configured in the [general configuration guide](/docs/iot-gateway/configuration/).  
...
1. Create a shared attribute in the device on WinstarCloud, to do this we go to the **ATTRIBUTES** tab, select from a list of attributes option "Shared attributes" instead of "Client attributes", press plus icon, put "sharedName" as Key and "New_name_" as "String value".
