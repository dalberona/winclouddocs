...
1. "name" - name of the device for WinstarCloud instance.  
...
7. "attributesUpdates" - objects array with a configuration for processing attribute update request from WinstarCloud.
...
```bash
/var/lib/winstarcloud_gateway/extensions
```
{: .copy-code}
...
|-|-|
| **sudo pip3 install winstarcloud-gateway** | `/usr/lib/python3/site-packages/winstarcloud_gateway/extensions` | Package installed on system layer, for every user. |
| **pip3 install winstarcloud-gateway** | `/usr/local/lib/python3/dist-packages/winstarcloud-gateway` | Package installed only for current user. |
...
from winstarcloud_gateway.connectors.connector import Connector, log    # Import base class for connector and logger
from winstarcloud_gateway.tb_utility.tb_utility import TBUtility
...
class CustomSerialConnector(Thread, Connector):    # Define a connector class, it should inherit from "Connector" class.
    ...
    def on_attributes_update(self, content):    # Function used for processing attribute update requests from WinstarCloud
        log.debug(content)
            ...
            if device_config is not None:
                ...
                if device_config.get("attributeUpdates") is not None:
                    ...
                    for request in requests:
                        attribute = request.get("attributeOnWinstarCloud")
                        log.debug(attribute)
...
The purpose of the converter is to convert data from devices to the WinstarCloud format.
...
{% highlight python %}
from winstarcloud_gateway.connectors.converter import Converter, log    # Import base class for the converter and log ("converter.log" in logs directory).
...
class CustomSerialUplinkConverter(Converter):    # Definition of class.
    ...
    def convert(self, config, data: bytes):    # Method for conversion data from device format to WinstarCloud format.
...
This dictionary will be converted into json and gateway will send it to WinstarCloud instance.
...
```bash
sudo python3 -c 'from winstarcloud_gateway.gateway.tb_gateway_service import TBGatewayService; TBGatewayService("./tb_gateway.json")'
```
{: .copy-code}

You can check a status of the IoT Gateway by watch the logs in a folder that you provide in logs.json file.  
Default location of logs folder for the daemon - "/var/log/winstarcloud-gateway/"  
Default location of logs folder for Python module - "./logs/"  

### Step 6. Check a result in the WinstarCloud instance

To check the result, you should connect device, and go to the WinstarCloud UI into "Devices" tab.  
...
**on_attributes_update** -- gateway call it when receives AttributeUpdates request from WinstarCloud server to device with this connector.  
**server_side_rpc** -- gateway call it when receives ServerRpcRequest from WinstarCloud server.  
...
Method is being called when gateway receive AttributeUpdates request from WinstarCloud.  

```python
def on_attributes_update_method(self, content):
```
*self* -- current object.  
*content* -- dictionary with data from WinstarCloud server.  
...
*req_id* - Id of RPC request from WinstarCloud  
...
```
 ...
 - If with response in content should be any dictionary with content that you want send to WinstarCloud as response.
...
**convert** -- Method for conversion data from device format to WinstarCloud data format.  
...
In the example used to save the converter configuration and create a template for the result's dictionary with WinstarCloud data format

##### convert method

Method for conversion data from device format to WinstarCloud data format.
...
Gateway will convert this data into json and send it to the WinstarCloud instance.
