...
description: CAN protocol support for WinstarCloud IoT Gateway
...
This guide will help you to get familiar with CAN connector configuration for WinstarCloud IoT Gateway.  
  ...
  "devices": [
    {
      ...
      "attributeUpdates": [
        {
          "attributeOnWinstarCloud": "softwareVersion",
          "nodeId": 64,
          "isExtendedId": true,
          "dataLength": 4,
...
This subsection provides the list of configurations, each of them describes which bytes to get from CAN payload (array of bytes) and how to convert these bytes to a Winstarcloud attribute or a time series key.  
...
By types **bool**, **int** and **float** it is meant Winstarcloud **boolean**, **long** and **double** types respectively.
...
This subsection provides the list of configurations to subscribe for changes of Winstarcloud shared attributes.
...
This subsection provides the list of configurations to process RPC commands from a WinstarCloud server to a device.

| **Parameter**            | **Default value**     | **Description**                                                         |
|:-|:-|-
| **method**               |                       | Name of RPC command.       |
| response                 | **false**             | If true, [response](/docs/reference/gateway-mqtt-api/#server-side-rpc) will be sent to WinstarCloud.       |
| **nodeId**               |                       | CAN node (arbitration) id.                                           |
| isExtendedId             | **false**             | If True means extended CAN node (arbitration) id.                                                 |
| isFd                     | **false**             | If True means using CAN FD mode.                                  |
| bitrateSwitch            | **false**             | **Only for CAN FD mode** If True means a higher bitrate is used for the data transmission.                                         |
