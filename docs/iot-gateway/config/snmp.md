...
description: SNMP monitoring support for WinstarCloud IoT Gateway
...
This guide will help you to get familiar with SNMP Connector configuration for WinstarCloud IoT Gateway.  
...
This connector is useful when you have SNMP manager in your network and you would like to push the data to the WinstarCloud.    
...
|:-|:-|-
| deviceName    | **SNMP router**                         | Device name in WinstarCloud.                   |
| deviceType    | **snmp**                                | Device type in WinstarCloud.                   |
...
By default, the gateway uses an uplink converter to send received data from SNMP manager to WinstarCloud, but it is also possible to use a custom converter.  
...
| **Parameter**     | **Default value**              | **Description**                                                                                         |
|:-|:-------------------------------|-
| key               | **ReceivedFromGet**            | Attribute key in device on WinstarCloud.                                                                 |
...
| **Parameter**     | **Default value**             | **Description**                                                                                         |
|:-|:------------------------------|---------------------------------------------------------------------------------------------------------
| key               | **ReceivedFromTable**         | Telemetry key in device on WinstarCloud.                                                                 |
...
WinstarCloud allows the provision of device attributes and fetches some of them from the device application.
...
WinstarCloud allows sending [RPC commands](/docs/user-guide/rpc/) to the device connected to WinstarCloud directly or via Gateway.
 
Configuration, provided in this section is used for sending RPC requests from WinstarCloud to the device through the gateway.

{% capture rpc_variants %}
**There are 2 types of the RPC calls:**  
1. With reply, after sending request the gateway will wait for a response and send it to WinstarCloud.
