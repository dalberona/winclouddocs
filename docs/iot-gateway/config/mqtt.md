...
description: MQTT protocol support for WinstarCloud IoT Gateway
redirect_from: 
...
The connector is also able to push data to MQTT brokers based on the updates/commands from WinstarCloud. 

This connector is useful when you have local MQTT broker in your facility or corporate network and you would like to push data from this broker to WinstarCloud.
...
| clientId             | **WinstarCloud_gateway**  | This is the client ID. It must be unique for each session.                                  |
...
The device will be created and displayed in WinstarCloud based on the passed parameters.
{:refdef: style="text-align: center;"}
...
WinstarCloud allows sending RPC commands and notifications about device attribute updates to the device.
...
If your device is constantly sending telemetry data - WinstarCloud already knows how to push notifications.
...
Your WinstarCloud instance will get information from the broker about last connecting time of the device. You can see this information under the "Server attributes" scope in the "Attributes" tab.
...
Your WinstarCloud instance will get information from the broker about last disconnecting time of the device. You can see this information under the "Server attributes" scope in the "Attributes" tab.
...
In order to request client-side or shared device attributes to WinstarCloud server node, Gateway allows sending 
attribute requests.
...
This configuration section is optional.  
WinstarCloud allows to provision device attributes and fetch some of them from the device application.
You can treat this as a remote configuration for devices. Your devices are able to request shared attributes from WinstarCloud.
...
Run the command below to start the *mosquitto_sub* client, subscribing to the topic "sensor/SN-001/firmwareVersion" of the local broker. Start waiting for new messages from WinstarCloud server to broker.
...
Update device attribute value on the WinstarCloud server following these steps:
...
Broker received new message from the WinstarCloud server about updating attribute "FirmwareVersion" to "1.2".
...
WinstarCloud allows sending [RPC commands](/docs/user-guide/rpc/) to the device that is connected to WinstarCloud directly or via Gateway.
 
Configuration, provided in this section is used for sending RPC requests from WinstarCloud to device.
