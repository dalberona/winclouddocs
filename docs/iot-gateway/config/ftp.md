...
description: FTP protocol support for WinstarCloud IoT Gateway
...
This guide will help you to get familiar with FTP Connector configuration for WinstarCloud IoT Gateway.
...
The connector is also able to push data to FTP server files based on the updates/commands from WinstarCloud.

This connector is useful when you have some FTP server in your device or some data in an external resource and you would like to push this data to the WinstarCloud.    
...
WinstarCloud allows the provisioning of device attributes and fetches some of them from the device application. You can treat this as a remote configuration for devices, enabling them to request shared attributes from WinstarCloud. See [user guide](/docs/user-guide/attributes/) for more details.
