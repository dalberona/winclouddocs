...
- `tbmq-logs:/var/log/winstarcloud-mqtt-broker` - maps the `tbmq-logs` volume to TBMQ logs directory;
...
**Note**: In case the TBMQ is being installed on the same host where WinstarCloud is already running, the following issue can be seen:
...
```shell
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/winstarcloud/tbmq/{{ site.release.broker_branch }}/msa/tbmq/configs/windows/tbmq-upgrade.ps1" `
