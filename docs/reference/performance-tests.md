...
description: WinstarCloud IoT Platform performance tests
...
These AMIs contain some tuned OS parameters, for example, the maximum amount of threads per process and open file descriptors:
 ...
 - [WinstarCloud AMI](https://console.aws.amazon.com/ec2/v2/home?region=us-west-1#LaunchInstanceWizard:ami=ami-09b1ed69) (username **ubuntu**)
...
If you would like to verify performance for all components hosted on a single server instance, simply run WinstarCloud AMI instance.
By default this instance will be using Cassandra that runs locally.

If you would like to verify performance for standalone WinstarCloud server that uses external Cassandra Cluster, please init Cassandra cluster using provided Cassandra AMI first.
...
```bash
cqlsh 172.21.12.100 -f /usr/share/winstarcloud/data/schema.cql 
cqlsh 172.21.12.100 -f /usr/share/winstarcloud/data/system-data.cql 
cqlsh 172.21.12.100 -f /usr/share/winstarcloud/data/demo-data.cql 
```

Once Cassandra cluster setup is done please run WinstarCloud AMI instance. You need to update **winstarcloud.yml** config to use Cassandra cluster instead of a local instance:
...
After configuration update, restart WinstarCloud service:
...
Once you will setup your cluster configuration using WinstarCloud and Cassandra AMIs you can execute tests from "client" machines (WinstarCloud Performance Test AMIs) using following commands:
 
```bash
cd projects/performance-tests
```

Update **mqttUrls** and **restUrl** and set private IPs of AWS instance where WinstarCloud service is deployed in **test.properties** file:
